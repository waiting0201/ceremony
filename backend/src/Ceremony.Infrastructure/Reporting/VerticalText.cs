...
internal static class VerticalText
{
    ...
    /// 壓到樣板預印的「靈位」（6 位 × 5 字 → 鏈高 11 × 0.6 = 6.6cm，薦牌方框只有 5.4cm；
    /// 資料卡窗框 5.5039cm，同一組輸入越過「靈」上緣約 0.90cm）。
    /// 典型 3~4 字姓名（含 6 位滿版）鏈高 ≤ 9 × 0.6 = 5.4cm，兩份報表都與改動前逐位元相同。
    ///
    /// **只給「往者 3+ 位矩陣」用**：薦牌 <c>TabletRenderer</c> 與（2026-08-17 使用者指定
    /// 「資料卡也不縮字、跟薦牌規則一樣」起）資料卡 <c>DataCardRenderer</c> 的 3+ 位分支。
    /// 兩者的 **1／2 位** 分支、薦牌**陽上**、文牒仍走 <see cref="MatrixLayout"/>——這是規則本身
    /// （只有 3+ 位矩陣退出防線），不要順手把其餘呼叫端也改過來。
