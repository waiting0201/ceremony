...
last_updated: 2026-07-31 (B15 結案：使用者指定地址可整段清空，信眾表單的 Validators.required 已移除；同日稍早新增 B15：信眾表單「寄件地址」仍掛 Validators.required、與報名表單/後端 2026-07-21 的放寬不一致，要不要一併放寬；同日 B7 更新——「同寄件地址」門檻已依使用者指定放寬為「城市/區域/地址三者全空才擋」，剩「持續綁定」未做。先前 2026-07-29 (新增 B14：郵遞區號文字快照與區域 FK 不一致的 277/275 筆歷史列要不要重新對齊〔本次修正只回填「文字為空」的 164 筆，不一致者一律不動〕。先前 2026-07-21 B13 反轉為報名層級／方案 A：三欄 per-signup 可編輯，已實作；先前 2026-06-29 曾定案信眾層級/方案 C))
---
...
- **目前**：勾選複製 Mail → Text（城市/區域/地址一起）；**城市/區域/地址三者全空**才阻擋（2026-07-31 依使用者指定放寬，已刻意偏離舊系統的「必須先有地址文字」）
- **可能強化**：兩地址綁定（mail 改 text 自動跟著改）——目前仍是「勾選當下複製一次」，勾完再改寄件段不會同步
...
### B15. 信眾表單「寄件地址」是否也放寬為非必填？

- **現況不一致**：報名表單與後端已於 2026-07-21 依使用者指定放寬地址為非必填，但 `believer-edit-form` 的 `mailAddress` 仍掛 `Validators.required`（畫面上仍有 `*`）。同一份地址資料，從報名表單存得進去、從信眾維護頁卻被前端擋下
- **問題**：信眾主檔要不要跟著放寬？（後端 `BelieverWriteValidator` 已不擋空，所以放寬只是拿掉前端 required）
- **影響**：[believer-management.md](blueprints/believer-management.md)、[business-rules-implicit §12](business-rules-implicit.md)
- **狀態**：✅ **已確認並實作（2026-07-31）**——使用者指定「新增或修改資料時可以取消原資料有的寄件及文牒地址」，
  required 已移除、placeholder 的 `*` 拿掉。詳見 [business-rules-implicit §12.1](business-rules-implicit.md)
