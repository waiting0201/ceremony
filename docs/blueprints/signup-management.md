...
#### 編號改起迄區間（2026-07-31 使用者指定，**刻意不同於舊系統**）

- 搜尋面板的「編號」由單一 `nudSearchNumber` 改為**起 ~ 迄兩格**（各自帶 ▲▼ stepper），送 `numberStart` / `numberEnd`。
- **只填一端＝只搜尋那一筆**：另一端由前後端各自補成同值（前端 `buildQuery()`、後端 `SearchSignupsHandler.Normalize`），SQL 退化為 `Number >= n AND Number <= n`。兩端皆空＝不限編號（沿用舊系統 Number 空白/0 的語意）。
- **起 > 迄**：前端在 `search()` 送出前擋下並顯示「編號錯誤」（與批次列印同一句訊息）；後端不另擋（`>=`/`<=` 交集為空）。
- **批次列印同步放寬**：起迄兩格皆非必填，只填一端＝只印那一筆（`BatchReportHandler` 補同值）；兩端皆空才是 400「編號錯誤」。舊 `btnPrint_Click` 的 `nudStart`/`nudEnd` 兩格必填、起 > 迄跳 MessageBox。
- **取捨**：舊系統搜尋只能查單一編號，要看「1~50 號」得自己一筆一筆點；批次列印又硬性要兩格都填，想印單筆得把起迄打成同一個數字。兩處統一成「區間，只填一端就是單筆」後，單筆與區間都少一道手續，且兩個面板的輸入語意一致。
...
- **狀態保存**：`isAll` 併入 `SignupSearchFormSnapshot`，另存 `searchedBeforeAll` 於 `SignupSearchState` → 離開列表頁（edit / logs 或切到其他功能）再返回時模式與條件一併還原；stale 重查也會沿用全部模式。詳見下段。
...
#### 切到其他功能再回來要保持原樣（2026-07-31 客訴）

- **客訴**：報名維護勾了「範圍」、勾了「顯示完整表格」，切到別的功能再切回來，兩者都被取消勾選。
- **根因兩條**：
  1. `SignupSearchState` 的條件快照只在 `search()` 成功後才寫。使用者「勾了條件但還沒按搜尋」就切走 → 回來還原的是**上次搜尋當下**的條件，等於把剛勾的撤銷。
  2. 「顯示完整表格」是 `showAll` signal、不在 `SignupSearchFormSnapshot` 裡，**完全沒被保存**——2026-07-29 取消 localStorage 記憶時，連帶讓它變成「元件銷毀就沒了」。
- **修法**：(1) `bindFormSnapshot()` 訂閱 `form.valueChanges`，任何條件變動立即寫回 `state.form`（結果 / 總數 / 選取仍只在 `search()` 存，切走再回來不會憑空重打 API）；(2) `SignupSearchState` 新增 `showAll` 一格，`toggleShowAll()` 直接同步、元件建構時取回。
- **與 07-29「不記憶顯示完整表格」不衝突**：那條要的是「**開軟體**不要預設 32 欄」，本次要的是「**同一次執行期間**切走再回來別變」。兩者用「只存記憶體 singleton、不落 localStorage」同時滿足——關掉 App 即回預設。
- 回歸鎖：`signup-list-page.spec.ts` 「跨路由保留搜尋條件 / 檢視設定」（兩次 `createComponent` 共用同一個 injector ⇒ 同一個 singleton，模擬離開再回來）。
...
|---|---|---|---|---|---|
| 1 | 代入新增 | 列表 overlay（`editOverlay.fromSignupId`；`/signups/new?fromSignupId=` 路由僅保留為 deep link fallback，無程式導向） | **單選**（==1 才 enable） | 全部 | 對應 `tsmiAdd_Click` (cs:151)：把選列的 SignupID + Name 帶入 NewSignupForm。2026-08-05 起工具列「新增報名」在恰好選 1 筆時也走這條，見下方「新增報名的兩個入口與代入規則」 |
...
| 1c | **移動插入至…**（新版增強，無舊對應） | 對話框輸入目標編號 → `POST /api/v1/signups/{id}/move-number` | **單選**且該筆有編號 | 全部 | 把既有一筆移到同群組內的指定編號，中間區段自動 ±1 遞補。**與 1b 的分工**：1b 是新增一筆、1c 只是移位（總筆數不變、不留空號）。見下方「移動並重排」段 |
| 2 | 修改資料 | navigate `/signups/:id/edit` | **單選** | 全部 | 對應 `tsmiEdit_Click` (cs:168)：未選列 verbatim「尚未選擇報名資料」 |
| 3 | 列印資料卡 | `GET /api/v1/reports/datacard?signupId=` ×N（或前端拼批次） | 單 / 多選皆可 | **選取列全為普桌（4）時停用**（2026-08-15，提示改印普桌資料卡） | 對應 `tsmiPrintDataCard_Click` (cs:188)；型別互斥見下方「兩張資料卡互斥停用」 |
...
| 7 | 列印普桌（牌位） | `GET /api/v1/reports/worship?signupId=` | 單 / 多選 | **完全不鎖型別**（2026-07-18 客訴改）：前端恆啟用、後端選什麼印什麼 | `tsmiPrintWorship_Click` (cs:380) 本就無型別檢查；原新系統「限 type-4」自加防呆（422 `WORSHIP_ONLY_TYPE_4`／批次過濾）已全數撤回 |
| 7b | 列印普桌資料卡 | `GET /api/v1/reports/worshipcard?signupId=` | 單 / 多選 | **選取列全非普桌時停用**（2026-08-15，提示改印資料卡） | 後端仍完全不鎖型別（同 7）；這是純前端引導，見下方「兩張資料卡互斥停用」 |
...
**兩張資料卡互斥停用（2026-08-15 使用者定案）**：

| 選取內容 | 列印資料卡 | 列印普桌資料卡 |
|---|---|---|
| 全部都是普桌（4） | 停用「普桌報名請改印『普桌資料卡』」 | enable |
| 全部都不是普桌 | enable | 停用「非普桌報名請改印『資料卡』」 |
| 混選 | enable | enable |

- 理由：兩張紙同為 21×14.8cm、外觀相近，印錯不易當場察覺，但驅動 form 名不同、自動選紙不可互相替代。
  與新增報名那顆「列印資料卡」鈕（§16.1）同一套判斷
- **不是回到 2026-06-29 那版**：那版是單向鎖、會走到「什麼都印不了」才引發客訴；這版互斥，
  永遠至少有一張資料卡可印，停用項一律附 tooltip 指路
- 混選刻意不擋（跨型別批次是使用者明知的操作）；**後端契約完全不變**；列印預覽頁下拉不套此規則
- 實作：`signup-list-page.ts` 的 `dataCardGate`（右鍵與工具列「對選取項目」kebab 共用同一份 menu，
  故兩處行為一致）；回歸鎖 5 案於 `signup-list-page.spec.ts`。詳見 [business-rules-implicit.md §16.2](../business-rules-implicit.md)
...
**新增報名的兩個入口與代入規則（2026-08-05）**：

舊系統的工具列「新增」（`btnNew_Click:76-90`）與右鍵「新增」（`tsmiAdd_Click:151-166`）是**逐行重複的同一段程式**——有選取列就把 `SignupID` + `Name` 帶進 `NewSignupForm`（＝代入新增，同一位信眾續報下一場的主要動線）。新版原本把代入只留在右鍵，工具列寫死空白新增，現場照舊習慣操作會拿到空表單。現已對齊：

- **工具列「新增報名」**：`openCreateOverlay()` 在**恰好選 1 筆**時轉呼叫 `actionAddFrom(rows[0])`，沒選或選多筆才是空白新增。⚠️ **刻意偏離**舊 `selectedcount > 0 → SelectedRows[0]`：新系統多選是常態（批次列印動輒數百列），沿用會變成「隨機拿第一列代入」。用 `selectedRows()`（以 `results()` 過濾）而非 `selectedIds()`，搜尋換過後殘留的 id 自動失效。
- **代入後自動搜尋 + 選中來源列**：對齊舊 `btnNextStep_Click:97-111`「//代入新增」段——`prefillFromSignup` 取得來源報名後把姓名填進搜尋框 → 跑一次信眾搜尋 → 在結果中找到來源列 → 以它覆蓋表單並高亮。此前只做 `patchValue`，搜尋框空白、結果表不渲染、無高亮，使用者無法就地改選同信眾的別筆報名。姓名為空則不發搜尋（同舊 `if(ParamName != string.Empty)`），改走 fallback。來源列若被 200 列上限切掉會被 pin 到結果最前（**加值偏離**，舊系統無列數上限）。
- **共用一段實作**：代入與「使用者點結果列」都走 `applyBelieverRow(row, { markDirty })`，順帶修掉代入路徑先前的三處落差——預繳未帶、地址整段為空時未退回信眾主檔、`selectedBeliever` 只拿 stub。
- **代入後維持 pristine**：內容 100% 來自系統、使用者一個字都沒打，關 overlay 不該被「未儲存的變更」攔下（`pickBeliever` 標髒的理由是為了讓草稿存得下來，而代入模式本來就不存草稿）。
- **取捨：選 1 筆按工具列新增時不還原跨路由草稿**（該情境已非 `isPlainCreate`）。草稿**不會遺失**——`saveDraft()` 同樣被擋掉，既有草稿原封不動；逃生口是結果列上方的「取消選取」鈕，清掉選取再按新增報名即回到純新增。不採用「代入也還原草稿再蓋上代入值」：兩份資料來源混在一起，是既有原則明確排除的做法。
- 回歸鎖：`signup-list-page.spec.ts` 4 案（未選/選 1/選多/選取列已不在結果內）、`signup-edit-form.component.spec.ts` 8 案（搜尋+高亮、pristine、預繳、姓名為空、結果不含來源列、年/法會/類型不代入、代入後改選標髒、不存草稿）。
...
**移動並重排（新版增強，2026-08-21）**：
- 需求（現場原話）：「『在此前插入』用於新增一筆是可以的，但不適用於移位——現有 1-10 號，想把 5 號移到 2 號，
  插到 2 號後只能把原來的 5 號那筆刪除，中間就會空著一個編號。」故要第二種插入方式：
  **選 5 號 → 右鍵『移動插入至…』→ 輸入 2 → 原 2-4 各 +1**；反向**選 2 號 → 輸入 5 → 原 3-5 各 -1 補上前面的空號**。
- 語意：兩個方向一致＝**該筆佔據目標編號，中間區段整體讓位**。上移 `[to, from-1]` +1、下移 `(from, to]` -1、同號 no-op。
- 入口：右鍵 →「移動插入至…」（`signup-list-page.ts` `actionMoveNumber`，icon `move-vertical`）。
  **恰好選 1 筆且該筆有編號**才啟用（寺方那類無編號的列停用，提示「請先選擇 1 筆有編號的資料」）。
  不開報名表單——只彈一個數字輸入對話框（`ConfirmDialogService.askNumber()`，預填目前編號）。
- 後端：`POST /api/v1/signups/{id}/move-number`（`MoveSignupNumberHandler` → `SignupRepository.MoveNumberAsync`）。
  單一交易內先讀該筆群組（applock 的 resource 名要用到 year/cat/type）→ `sp_getapplock`（與 insert-shift／預繳載入共用）
  → 鎖內取 MIN/MAX 驗範圍 → 一句 set-based ±1 UPDATE → 該筆佔位。詳見
  [post-signups-move-number.md](api-endpoints/post-signups-move-number.md)。
- **三項定案（2026-08-21 使用者拍板）**：
  1. 目標超出該群組現有 MIN..MAX → **擋下並提示「目前 N–M」**，不 clamp 成最後一號（照字面搬會留一段空號，與需求相反）
  2. **完全不寫 SignupLog**（移動那筆與被讓位的列都不寫）。代價寫在這裡以免日後誤判為 bug：**歷程看不出某筆被移過**
  3. **只做單筆**（多筆連續移動語意複雜——不連續怎麼辦、順序如何保留——且現場沒有需求）
- 範圍：只在同 `(年, 法會, 報名類型)` 內移動，不跨群組（跨群組＝換法會/類型，那是編輯報名的事）。
...
- 呼叫 `POST /api/v1/reports/batch` body `{ reportType, numberStart?, numberEnd?, year?, ceremonyCategoryId?, signupType? }`（起迄至少填一個；只填一端＝只印那一筆，見上方「編號改起迄區間」）
- 回 PDF blob → 開新分頁 / iframe 預覽（同 reports preview page 機制）
- **所有 reportType（含普桌／普桌資料卡）一律跟隨當前 signupType filter，不強制 SignupType=4**（2026-07-18 解鎖，見 [business-rules-implicit §16](../business-rules-implicit.md)；此處原記「普桌強制 SignupType=4」為解鎖前殘句，2026-08-12 修正）
...
基本資料   堂號(可編輯 input) / 姓名 / 聯絡電話 ‖ 員工類型(可編輯 select) / 固定編號(可編輯 checkbox)
           （三欄 2026-07-21 改 per-signup 可編輯；2026-07-31 使用者指定改為此順序，
            ...
            同日再指定：員工類型下拉寬度＝下方右欄「費用」欄寬——右半改用與費用所在
            `.grid.three` 相同的三等分軌道，下拉佔第一格、固定編號吃第 2~3 格）
...
- **選信眾帶入預繳＝取該筆報名自身值（2026-07-31 客訴，取代原本的「查最新一筆」）**：`pickBeliever` 直接帶點到那一列的 `prepayYear` / `prepayCeremonyCategoryId`（該列沒預繳就空白），**不再呼叫 `GET /prepay?believerId&year`**。原作法一律帶「該信眾今年以前最新一筆報名」的預繳且**完全不分報名類型**，同一位信眾點法會列與普桌列（`SignupType` 4）都拿到同一份，普桌明明沒預繳卻顯示有——而法會與普桌是分開報名的兩件事，法會預繳 ≠ 普桌預繳。**刻意偏離舊系統**（舊 `BelieverSelected:1102-1115` 同樣沒有類型過濾）。見 [business-rules-implicit §19](../business-rules-implicit.md)、[get-prepay-believer-latest.md](api-endpoints/get-prepay-believer-latest.md)（endpoint 保留但已無呼叫端）
- **新增報名頁客訴六項（2026-07-21）**：
  ...
  2. **改選信眾殘留欄位修復**：`pickBeliever` 覆蓋整張表單時預繳（預繳年/預繳法會）一律重寫，避免沿用前一筆。**2026-07-31 起改為直接取該列自身的預繳**（原為「先歸零再由 `prefillPrepayHistory` 查最新一筆回填」，該作法不分報名類型、會把法會預繳帶到普桌，見上方「選信眾帶入預繳」）。**編號欄（keepNumber/customNumber）2026-07-31 起改為不清**（見下方客訴第四輪第 4 項）。**備註 2026-07-27 起改為帶入該筆報名的 `remark`（無則空字串）**，不再一律清空（見下方「選定回填」）。**費用 2026-07-27 起改為不清（使用者指定，取代原本一併清空的作法）**：費用不會從結果列帶入，唯一來源是使用者自己輸入，改選時清掉等於吃掉已打好的金額；舊 `BelieverSelected` 也完全沒碰 `txtFee`（只在 `btnConfirm_Click:236` 讀值 + `txtFee_Validating` 數字驗證），故**不清才是對齊舊系統**
...
- **新增報名表單客訴五項（2026-07-31，第四輪）**：前三項是版面（細節與理由見 [frontend-design.md「報名表單版面（2026-07-31 客訴第四輪）」](../design/frontend-design.md)），後兩項是行為。
  ...
  3. **基本資料改序 堂號→姓名→電話→員工類型→固定編號**，前三欄與地址框逐像素等寬；**同日追加：員工類型下拉寬度＝右欄「費用」欄寬**（同樣靠「與費用同一組三等分軌道」達成，不寫死欄寬）
  ...
  - **信眾表單同步**（使用者指定）：第 1、5 項一併套到 `believer-edit-form`（往生/陽上本來就有外框；第 3、4 項是報名表單獨有）。✅ 該表單遺留的 `mailAddress` `Validators.required` 已於 2026-07-31 移除（使用者指定地址要能整段清空），與報名表單/後端一致；見 [business-rules-implicit §12.1](../business-rules-implicit.md)
...
- **填到一半切走不能被清掉＝跨路由草稿（2026-07-27 客訴）**：新增報名是 overlay（backdrop `position:fixed; inset:0` 蓋滿全螢幕），要點側欄切到其他功能頁**一定得先關掉 overlay** → 表單元件被銷毀 → 填到一半的內容全沒。修法：新增 root singleton [`SignupDraftState`](../../frontend/src/app/features/signups/signup-draft-state.ts)（比照既有 `SignupSearchState`），元件銷毀時存、下次開啟時還原。**使用者定案的三項行為**：
  ...
  - **草稿內容**：整份 `form.getRawValue()`（含法會資料/信眾/地址/名單/編號/費用/備註/預繳）＋ 已選信眾摘要、常駐搜尋列表的關鍵字/結果/高亮列、`lastCreatedSignupId`（＝「列印資料卡」鈕的啟用狀態）、`lastCreatedSignupType`（＝該鈕印一般資料卡還是普桌資料卡，2026-08-12）、`dirty` 旗標。**不入草稿**：城市→區域下拉的選項（還原時重跑 `applyAddress` 連動重新載入）與重複報名警示（`valueChanges` 會自動重查）
...
- **「列印資料卡」（2026-07-27 補回，對齊舊 `btnPrintDataCard`）**：新增類在 **overlay 底部 actions 列、「取消」按鈕左邊**（同日使用者指定位置；`/signups/new` 路由頁的按鈕列同樣處理），**存檔前 disabled**（無額外提示文字）、**新增成功後啟用**——完全對齊舊 `NewSignupForm`（進 Step2 `btnPrintDataCard.Enabled = false`:95 → `btnConfirm` 成功 `= true`:361）。點擊後以「剛新增那一筆」的 id（＝舊 `CurrentSignupID`，存於表單的 **public** `lastCreatedSignupId` signal，供 host 判斷 disabled；host 端用 `viewChild()` signal query 取表單，`@ViewChild` 非 reactive 在 OnPush + zoneless 下不會即時更新）打既有 `GET /reports/datacard?signupId=`，PDF 開新分頁預覽（共用 `shared/util/pdf.ts` 的 `openPdfInNewTab`，自 signup-list-page 抽出）。按「取消」（＝清成新的一筆）會把它重新 disable。**編輯模式不顯示**——既有報名要列印走報名維護的右鍵選單
  - **卡別依報名類型自動分流（2026-08-12 使用者定案，新版加值）**：普桌（`SignupType = 4`）打 `GET /reports/worshipcard?signupId=`（普桌資料卡），其餘型別維持 `GET /reports/datacard?signupId=`；**按鈕文字同步變**「列印普桌資料卡」／「列印資料卡」，讓使用者按之前就知道會印哪一張（兩張紙同為 21×14.8cm，印錯不易當場察覺）。
    判斷來源是**存檔當下的類型快照**（`lastCreatedSignupType` signal，與 `lastCreatedSignupId` 同時寫入、同時清空、同樣入草稿）而非下拉即時值——存完表單不關閉、資料仍留在畫面上，使用者可能改了「報名類型」卻沒再按確認，而這顆鈕印的是**已存的那一筆**。
    舊系統 `btnPrintDataCard_Click`（NewSignupForm.cs:371-398）永遠 `tmpDataCard.rdlc`，因為舊系統沒有「普桌資料卡」這張報表，故此為新版加值、非反推。與右鍵選單「選什麼印什麼、不檢查型別」不衝突（那是明示選擇、這是系統代選），見 [business-rules-implicit §16／§16.1](../business-rules-implicit.md)。
    回歸鎖：`signup-edit-form.component.spec.ts`「列印資料卡：普桌報名改印普桌資料卡，按鈕文字同步變」
...
- [ ] **「列印普桌」（牌位）不 grey out**：有選取即 enable；選什麼印什麼（對齊舊系統）
- [ ] **兩張資料卡互斥**：全普桌鎖「列印資料卡」、全非普桌鎖「列印普桌資料卡」、混選皆可（2026-08-15）
