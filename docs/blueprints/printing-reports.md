...
> **適用對象（2026-08-12）**：新增報名表單的「列印資料卡」鈕只在**非普桌**（`SignupType` ≠ 4）時走本報表；
> 普桌改走 §6 普桌資料卡。列表右鍵「列印資料卡」不分型別、維持選什麼印什麼。見 [business-rules-implicit §16.1](../business-rules-implicit.md)
...
- **入口二：新增報名表單的「列印資料卡」鈕自動選用（2026-08-12 使用者定案）**——該鈕依剛新增那筆的
  ...
  與上一條不衝突：上一條講的是**使用者明示選擇**的入口（右鍵／預覽頁）不得被系統否決，
  這條講的是**系統代選**時要選對。⚠ 兩張卡紙張同為 21×14.8cm 但驅動 form 名不同
  （「資料卡」／「普桌資料卡」），自動選紙不可互相替代（見 `PrinterFormMatcher`）。
  詳見 [business-rules-implicit §16.1](../business-rules-implicit.md)、[signup-management.md](signup-management.md)
