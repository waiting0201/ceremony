...
### 「造字」＝ Unicode 增補平面罕用字（**2026-08-04 修正**）

客訴「舊系統的造字印得出來，新系統只有收據、資料卡陽上正常，其他都亂碼」。**不是字型問題，是逐字拆字的直書排版把字拆壞了。**

- **這些字是什麼**：不是 Big5 造字區（PUA）。實查 dev DB，`Signups.Name` 7 筆、`Believers.DeadNameOne` 7 筆含 **CJK Ext-B（U+20000 以上）** 罕用字（`𡍼`／`𤆬`／`𣸸`／`𥰁`…），PUA 區 **0 筆**。這種字在 `nvarchar` 佔 **兩個 UTF-16 code unit（surrogate pair）**。
- **壞在哪**：`VerticalText.Stack` 的 `foreach (var ch in text)` 會在 surrogate pair 中間插 `\n`，兩半各自變成無效碼位；`GroupFontPt` / `MatrixLayout` 的 `name.Length` 把一個字算成兩列，連帶把整組字級縮錯；`SkiaImageHelpers.VerticalAddress` 的 `text[i]` 同一個病。
- **為什麼只有收據與資料卡陽上倖存**：那兩處是**唯一整串交給 QuestPDF 橫書輸出**的欄位，字沒被拆，缺字時 Skia 還會自動 fallback（實測收據 PDF 內 `家歷` 用 BiauKai、`𡍼` 用 PingFangTC）。報表沒有「壞的」與「好的」之分，只有「逐字拆」與「不拆」之分。
- **修正**：`VerticalText` 新增 `Elements()` / `ElementCount()`（text element／字素），`Stack` / `GroupFontPt` / `MatrixLayout` / `SkiaImageHelpers.VerticalAddress` / `AddressColumns` / `ReceiptRenderer.CoverAddressLineCount` 全改以字素為單位。**本層之後不得再出現 `foreach (var ch in ...)` 或姓名的 `.Length`。**
- **Skia 那條另需缺字 fallback**：`canvas.DrawText` 用單一 typeface、**完全沒有 fallback**，標楷體沒有的字會整字畫不出來（留空白，比豆腐更難察覺）。已加 `SKFontManager.Default.MatchCharacter` 逐字素補字型；**行高／步進／字級一律沿用標楷體 metrics**，fallback 只換字形不換版面，故地址欄位不位移。
- **驗證**：`pdftotext` 產出不得含 U+FFFD 且必須找得到該字（薦牌／文牒／資料卡三張實測 0 個 U+FFFD）；垂直地址 PNG 以像素掃描確認該字有墨。回歸鎖見 `RendererSmokeTests` 造字段（7 條）。
- **連帶**：修正後含造字的姓名列數變少 → **字級會變大、下排起點會變**（`MatrixLayout`）。這批座標屬 [printing-reports-positions.md](printing-reports-positions.md) 的零容忍範圍，**含造字的實體樣張需重新複驗**。
...
### 文牒 2 位往生者置中於堂號中軸（2026-08-17）

客訴（附截圖 [reference/文牒.png](../../reference/文牒.png)）：「文牒往生者 2 位時，名字要往右一點，可以在兩邊堂號的置中位置」。

**根因是下面 07-27 那輪的兩個決定疊起來**：Two 變體的最左欄被硬錨到 `DeadLeftX`（「離左側預印字 0.5cm」的錨點），而它兩欄的相對距只有 RDLC 的 1.16299cm——比 Base 三欄的 2×0.91251 窄。結果整組墨跡帶 11.707~13.670、中心 **12.6885**，比堂號中軸 **13.0258** 偏左 0.337cm。

關鍵查證：**只有 Two 是離群值**。Base 1 位（主欄）中心 13.020、Base 3~6 位（三欄）13.0195，與堂號中軸只差 0.006cm ⇒ 客戶要的置中**本來就是既有設計**，這輪是把 Two 拉回同一條線，**Base 完全不動**。

**改法**：`TextRenderer` 新增 `HallFontCm`（0.6）／`HallCenterX = (HallSecondX + HallFirstX + HallFontCm)/2`／`DeadTwoColOffset`（1.16299），移除常數 `DeadTwoRightX`，改為 helper `DeadTwoColumnsX(fontCm)` = 中軸 − (欄距 + 字寬)/2。0.8cm 時 Left 由 11.707/12.870 變成 **12.04427/13.20726**；Top、字級、`MatrixLayout` 呼叫、欄高皆不動。堂號中軸與字數無關——它是**橫排**欄位、`SplitHallName` 每格只給 1~2 字，2 字在 0.7cm 欄內換行成上下兩列，墨跡寬恆為 0.6cm。

**依實際字級回推、不寫死位移量**：呼叫排在 `MatrixLayout` 算完字級之後，對齊 [gotchas](../gotchas.md)「套印置中不可用固定位移量，必須中軸 − 組寬/2」那條（＝資料卡 2026-07-27「1／2 位沒置中」二度客訴的結論），與 `DataCardRenderer.DeadColumnsX` 同一套做法。

⚠️ **副作用（刻意）**：Two 變體最左欄離預印字自 0.5cm 變成 **0.837cm**。0.5cm 是「不要壓到預印字」的**下界不是等式**，往右移是遠離它，**不是**退回 07-21 被推翻的 `DeadShiftX`；`DeadLeftX` 本身未動（Base 三欄與堂號左字仍靠它）。右緣 14.007 < Base 最右欄 14.332 < 硬界限 14.5。

200 DPI 回掃驗證：`text_debug_overlay_two.pdf` 往者墨跡 12.103~13.919（中心 **13.011**）、`text_debug_overlay_six.pdf` Base 三欄 11.760~14.249（中心 **13.005**）、堂號「堂」「甲」純黑墨跡 11.773~12.230／13.868~14.211（中心 **12.992**；「甲」字身窄造成的字型側距差，非版面誤差）——三者落差 ≤0.02cm。dotnet **335/144/74 綠**；回歸鎖 `Text_TwoDeadNames_AreCenteredBetweenHallNames`（0.8/0.7/0.6 三種字級都必須置中＝**反鎖**「寫死位移量」的實作，並一併鎖住 Base 同軸）。**待客戶實體套印複驗。**

**同輪一併補上：堂號 ≥2 字格上移 0.2cm**（`TextRenderer.HallTopOf`）—— 這是薦牌 2026-08-08 修過、文牒漏掉的**同一個 bug**，兩張報表的堂號格**完全同型**（0.7×1.3825cm、0.6cm 字、橫排 + `vMiddle`）。根因同：0.6cm 字塞 0.7cm 欄寬 → 第 2 字被 QuestPDF 換行往下長，但 `vMiddle` 只用單字高算位移（`top + (height − fontCm)/2`）→ 整塊重心下沉；這同時是**偏離 RDLC**，RDLC 的 `VerticalAlign=Middle` 是把換行後的整塊置中。實際 Top（含 `DeadShiftY`）：單字格 **2.6**、≥2 字格 **2.4**；逐格獨立判斷、以**字素**計。

⚠️ 補償量沿用薦牌那個**客戶實體套印認可過的 0.2cm**，不是幾何全補的 0.3cm（(n−1)×0.6/2）—— 格子同型時，客戶眼睛看過的值比自己新算一個安全，也讓兩張報表行為一致（回歸鎖直接斷言兩者補償量相等）。沿用薦牌的已知不足：3 字以上（整串進 First）也只補 0.2。

截圖那筆客戶資料正是 4 字堂號「江蘇阜寧」（每格 2 字）。回掃 `text_hallname_4chars.pdf`：兩格**各確實印出 2 列**（「窄欄靜默丟字」的高風險形狀，此前文牒零覆蓋——既有測試只用單字「甲」「堂」，與薦牌 08-08 發現的情況一模一樣），墨跡塊中心 3.591 → **3.391**（正好上移 0.2），距預印格中心 3.291 尚餘 0.1。回歸鎖 `Text_HallName_MultiCharCell_ShiftsUp`。dotnet **335/145/74 綠**。**待客戶實體套印複驗。**

### 文牒兩項客訴修正（2026-07-27）

客訴：「1. 陽上跟往者不管幾位，字體大小都跟一位一樣　2. 往者要跟左邊文牒本來的文字距離 0.5 公分，堂號也是同樣對齊」。

1. **字級不隨人數縮小** — 根因是 2026-07-21 那輪引進的「固定列距 + `WithBottomGap` 補全形空格 + `GroupFontPt` 以**列距**當可用高」：同欄上下排都有名字時可用高被限在列距（往者 2.06375cm／陽上 1.98436cm），3 字名 +1 列間距＝4 列 → 整組縮到 ≈0.50cm，人一多字就明顯變小。改用 [`VerticalText.MatrixLayout`](../../backend/src/Ceremony.Infrastructure/Reporting/VerticalText.cs)（**與薦牌 2026-07-17 同一套做法**）：字級以 `NameBaseFontCm`＝0.8cm 起算，**下排起點動態＝「上排（有下排配對者）最長字數 + 1 個字高間距」**，只有「整欄鏈」超出**整欄高**（往者 10.50374／陽上 6.72806cm）才整組等比縮。3 字名整欄鏈只要 7 × 0.8 = 5.6cm，兩個框都綽綽有餘 → **1~6 位一律 0.8cm**，上下姓名間距仍恰好一個字高（語意不變，只是改由 offset 而非補空格達成）。副作用：下排不再是固定 Top（往者 5.72264／陽上 17.25916）。
2. **「距離 0.5 公分」是間距、不是位移** — 07-21 把同一句客訴實作成 `DeadShiftX = 0.5`（整組**右移** 0.5cm）。RDLC 原座標下往者最左欄離左側預印字本來就有 0.293cm → 右移後實際間距 **0.793cm**，所以客戶再度回報（踩雷詳見 [gotchas](../gotchas.md)）。改成**絕對錨點**：量測 `reference/template/文牒.jpg`（2866×2023px ↔ 36.5×26.2cm，姓名帶 y 3.6~14cm 逐欄墨跡掃描）得往者左側最近的預印字欄「鳴呼既追攀之無從…」x **10.558~11.207cm** → `DeadLeftX = 11.207 + 0.5 = 11.707cm`。**Base / Two 兩變體都以「自己的最左欄」對齊此錨點**（Base 原 11.50、Two 原 11.85），欄與欄的相對距離維持 RDLC 原值。
3. **堂號「同樣對齊」**：`HallNameSecond`（左字）左緣對齊同一個 `DeadLeftX`，`HallNameFirst`（右字）維持 RDLC 相對距 2.03753cm → 13.744。縱向 `DeadShiftY=0.5` 與陽上 `LivingShift` 1cm 皆不動。

200 DPI 回掃驗證（`text_debug_overlay_six.pdf`）：預印欄墨跡右緣 **11.198**（與量測 11.207 吻合）／最左往者欄墨跡 **11.756**（框 11.707 + 字身邊距）／堂號「堂」**11.769**（與往者差 0.013cm）；Two 變體最左欄同為 11.756；6 位滿版字高 **0.685cm**（＝0.8cm 字級的墨跡高）、上下排間隔 ~1 個字高、未超出方框。回歸鎖 `Text_NameFontSize_StaysAtBase_RegardlessOfNameCount`、`Text_DeadNamesAndHallName_KeepHalfCentimeterFromPrePrintedText`；`CEREMONY_PDF_DUMP` 另落地 `text_debug_overlay_six.pdf`／`text_debug_overlay_two.pdf`。**待客戶實體套印複驗。**
...
- **入口（⚠️ 2026-08-08 起改為僅 API）**：走 `GET /api/v1/reports/tablet?signupId=...&debugGrid=true`，印的是**那一筆真實資料** + 1cm 桃紅刻度格線（`TabletRenderer.DrawCalibrationGrid`，橫向 0~11、縱向 0~25 各一條並標數字）。檔名帶 `-calibration` 尾綴，好與正式版並排比對。
  - **原本的右鍵選單項「列印薦牌（對位校正）」已移除**（2026-08-08，使用者要求）：**它是量測工具不是報表，不該出現在客戶的選單裡**——客戶看到一個印出滿版桃紅格線的「列印」選項只會困惑或誤按，而右鍵選單是報名維護的日常操作面。08-06 那次客訴的根因（2 位分支 `avail=6.31`）已結案且**與量測基準無關**，這個工具當下沒有現場需求撐住它佔一格選單。
  - **能力完整保留、只拿掉 UI 入口**：後端 `debugGrid` 參數、`PrintService.printSingle(type, id, { debugGrid })`、`ReportApi.single(..., { debugGrid })` 與四道回歸鎖全部不動。下次再遇到「算不出重疊卻實印壓字」時的兩條路：①遠端協助直接打上面那個 URL；②把選單項加回 `signup-list-page.ts`（該處留有指回本段的註解，加回來就是一個 `ContextMenuItem`，`onClick` 呼叫 `print.printSingle('tablet', id, { debugGrid: true })`）。
  - **教訓**：「現場量測工具不能 dev-gate」（下一條）解決的是**能不能在客戶機器上跑**，不等於**要不要放進客戶的日常 UI**。這兩件事分開處置：能力留在 API 層（隨時可用），入口只在需要時才開。
...
- ~~堂號（HallName）不印——已從 `DataCardData`/`DataCardModel`/`ReportModelBuilders.DataCard` 整條移除，不是保留欄位只是不畫~~ ⚠️ **已被 2026-08-14 客訴推翻，現況＝要印，見下方「資料卡補印堂號」**
...
### 資料卡補印堂號（2026-08-14 客訴）

客訴：「資料卡列印 右邊的列印沒有印到堂號　參考一下薦牌的列印方式」。

**根因不在座標，在管線**：2026-07-03 那次改版依樣板量測判定「資料卡印刷紙張沒有堂號欄」，把 `HallName` **整條移除**——`DataCardModel` / `DataCardData` / `ReportModelBuilders.DataCard` / `QuestPdfReportRenderer.RenderDataCard` 全部沒有這個欄位，`DataCardRenderer` 自然也沒有任何一行畫它。薦牌與文牒則一直都有（`SignupReportContext.SplitHallName(s.HallName)` → 兩個 `DrawText`）。修法是把四層一起接回來，不是調座標。

**版面（使用者選定）**：比照薦牌——堂號拆兩半、**分列右側窗框內「故」字的左右兩側**（右＝`First`、左＝`Second`，直書右起故右邊先讀）。資料卡窗框頂（4.394）到「故」字頂（4.585）只有 0.19cm，塞不下堂號，「故」左右是唯一可行、也與薦牌／文牒視覺一致的位置。

座標（`DataCardRenderer`，全部具名常數，單位 cm；皆已含 `FrameShiftX = 0.8`）：

| 項目 | 值 | 說明 |
|---|---|---|
| 可用帶上緣 `HallBandTop` | 4.494 | 框頂 4.394 + 0.1 內留白 |
| 可用帶下緣 `HallBandBottom` | 5.7388 | ＝`DeadTopRowY`（往者矩陣上緣），**硬邊界** |
| 左欄（`Second`）X | 15.9505 @0.6cm | 在「框內緣 15.773 ↔『故』左緣 16.728」空白帶置中 |
| 右欄（`First`）X | 18.0055 @0.6cm | 在「『故』右緣 17.828 ↔ 框內緣 18.783」空白帶置中 |
| 字級 | 起點 0.6cm | 同薦牌／文牒；用 `GroupFontPt` 只縮不放大 |
| 垂直 | 每側各自置中於可用帶 | 左右字數可能不同（3 字／5 字以上整串進 `First`） |

`DeadTopRowY` 由堂號與往者矩陣**共用**（原本是 `DrawDeadNamesInWindow` 內的區域常數），避免「同語意散成多個常數、錯一個沒人發現」——這正是 2026-08-06 薦牌壓字客訴的根因型態。

**繪製手法刻意與薦牌不同**：薦牌堂號用橫排 `DrawText` + 窄欄自動換行擠出「一格兩字」，因此需要 `TabletRenderer.HallTopOf` 那個「≥2 字上移 0.2cm」的補償（`vMiddle` 只用單字高算置中，不知道實際渲染成兩列 → 重心下沉）。資料卡改用既有的 `DrawVerticalName`（`VerticalText.Stack` 每字一行、`LineHeight(1f)`），列數明確 → 直接以實際字數置中，1／2／3 字都自然正確，**不該再抄那個補償，也不要為了「跟薦牌統一」改回橫排**。

**碰撞複核**：堂號右欄（18.0055~18.6055）水平上會與 3+ 位往者矩陣右欄（17.735~18.335）及 1-2 位往者的動態欄位重疊，但垂直上堂號下緣 ≤ 5.7388 ＝往者上緣，不相撞（薦牌是同樣的安排：堂號下緣 7.69 vs 矩陣上緣 7.7946）。

**回歸鎖**：`DataCard_HallName_SitsBesideGuGlyphAndStaysAboveDeadNames`（2/4/3 字三種長度的落點與邊界）、`DataCard_HallName_IsActuallyDrawn`（真的畫出來 + 落地樣張）、`DataCardHallNameTests`（builder 套用與薦牌同一支 `SplitHallName`）。

**⚠️ 判讀樣張要看純列印版**（`datacard_hallname_*_plain.pdf`）：`debugOverlay` 疊的樣板照片是 `FrameShiftX` 右移 0.8cm **之前**的原位置，會出現兩層「故」與兩層框線，堂號左半看起來像壓在（樣板那層的）「故」上，實際沒有。

**可轉移教訓**：依樣板量測判定「這張表沒有這個欄位」時，**優先「保留欄位、只是不畫」，不要把欄位從 model 整條移除**。移除當下省下的是幾行程式碼，日後客訴要回頭加卻是跨 4 層（Model / DTO / Builder / Renderer）的改動，而且症狀（「沒印到」）會誤導人往座標方向查。已記入 [gotchas](../gotchas.md)。
...
- 回歸測試：`DataCard_SixDeadNames_MatrixStaysWithinMeasuredWindow`（滿 6 位亡者含長名字驗證矩陣不超框；⚠️ 已於 2026-08-17 改寫成反向鎖並更名，見下節）、`DataCard_SixDistinctDeadNames_MatrixColumnsDoNotTouch`、`DataCard_OneDeadName_MatrixCenterTopRenders`（典型單一亡者情境）；PDF 存於 `reference/output/{datacard_six_dead_matrix_overlay,datacard_six_distinct_dead_overlay}.pdf`

### 資料卡往者 3+ 位不縮字（2026-08-17 使用者指定）

使用者指定：「現在資料卡也不縮字，跟薦牌規則一樣」。這是 2026-08-14 薦牌「3~6 位往者不用因為字數再縮小」的範圍擴大——那次刻意只動薦牌（`MatrixLayout` 的其餘三個呼叫端不碰），本次把資料卡也納進來。

**「跟薦牌規則一樣」＝依槽位分流，不是全部不縮**。薦牌的規則本來就只有 3+ 位矩陣退出防線：

| 往者槽位（`PrintTemplateSelector.SlotTier`） | 資料卡 | 薦牌 | 是否再縮 |
|---|---|---|---|
| 1 位 / 2 位 | `MatrixLayout`（可用高 5.5039cm） | `GroupFontPt`（可用高 `DeadGapHeight` 5.6cm） | **會** |
| 3+ 位 | `MatrixLayoutNoShrink` | `MatrixLayoutNoShrink` | **不會**（恆＝`ParaFontSizeCm` 0.6cm） |

1／2 位保留縮字**不是漏改**：2026-08-06「往者壓到預印靈位」客訴的實際路徑正是 2 位分支，那條防線兩份報表都完整保留。起點字級（`ChooseTablet` 的 `ParaFontSize`）自 2026-07-21 起就已同源，本次不動。

**⚠️ 代價（使用者明確選的取捨，與薦牌同型）**：資料卡窗框可用高 5.5039 ÷ 0.6 ≈ 9.17 ⇒ **鏈高 ≥10 單位就會越過「靈」上緣（11.4427）**，6 位 × 5 字（11 單位）壓約 **0.90cm**（薦牌同組輸入約 1.0cm）。**不要把它「修正」回 `MatrixLayout`**——反向鎖擋在那裡就是為了讓這種好心修改會失敗。典型 3~4 字姓名（含 6 位滿版，鏈高 ≤ 9 單位）**逐位元不變**。

**未同步的一項**：`FewDeadColumnGap` 仍是 0.1cm（薦牌 2026-08-14 加寬到 `DeadColumnGap` 0.3cm）。那次指示範圍只有薦牌，且資料卡窗框較窄（≈3cm），維持原值。

**驗證**：dotnet **335/143/74** 綠。200 DPI 回掃（`datacard_six_dead_matrix_plain.pdf`，6 位 × 5 字）對比改動前基準——列距 0.495 → **0.584~0.635**、字高 0.44~0.46 → **0.50~0.56**（＝0.6cm 字級的墨跡高）、下排起點 8.750 → **9.385**（理論 9.3388 + 字身邊距），與公式吻合。

**回歸鎖**：`DataCard_SixDeadNames_MatrixStaysWithinMeasuredWindow` 改寫成 `DataCard_ThreePlusDead_KeepsBaseFontSize_EvenWhenOverflowing`（**反向鎖**：斷言字級真的沒縮、舊公式在同組輸入下會縮、典型姓名兩公式逐位元相同——不是把斷言刪掉了事）；新增配對的不對稱鎖 `DataCard_1and2Dead_StillShrinkToFitWindow_MatrixDeliberatelyExempt`。**待客戶實體套印複驗。**
...
### 不做什麼
- HTML 印表機驅動（仍用後端 PDF）
  ※ 2026-07-31 起送印改由 Electron 主行程帶著紙張／邊界／縮放參數執行，不再是使用者自己在 PDF 檢視器裡按列印
  — 見 [print-channel-electron.md](print-channel-electron.md)
- 雲端列印
...
欄位（2026-08-14 起）：Number / Prepay / 5×LivingName / 5×DeadName（印進右側樣板窗框）/ Address / Phone / Remark / **HallName**（拆兩半、分列窗框內「故」字左右，見下方「資料卡補印堂號」；2026-07-03~08-13 期間曾整條移除）

> **適用對象（2026-08-12、2026-08-15 修訂）**：新增報名表單的「列印資料卡」鈕只在**非普桌**
> （`SignupType` ≠ 4）時走本報表；普桌改走 §6 普桌資料卡。
> **列表右鍵「列印資料卡」自 2026-08-15 起在「選取列全為普桌」時停用**（提示改印普桌資料卡），
> 混選與非普桌可印。見 [business-rules-implicit §16.1 / §16.2](../business-rules-implicit.md)
...
>
> ✅ **2026-07-21 客訴三項微調**（見 [DataCardRenderer.cs](../../backend/src/Ceremony.Infrastructure/Reporting/DataCardRenderer.cs)，座標詳表見 [printing-reports-positions.md](printing-reports-positions.md) §1）：(1) **編號抬頭與號碼分開繪製**——不再用「.」相接，抬頭起 `Left=1.361`、與號碼間留**固定 0.3cm 空隙**（`ReportModelBuilders.DataCard` 改傳 `NumberTitle`+`Number` 兩欄；`DrawNumberField` 用 QuestPDF `Row`+`Spacing(0.3cm)` 依抬頭**實際渲染寬度**定位號碼，半形抬頭 "No" 與全形 "信" 空隙皆準——不可用「字數×1.0cm」估算）；(2) **往者字級邏輯改與薦牌一致**——起點字級由 `PrintTemplateSelector.ChooseTablet` 決定（1-2 位 0.8/0.5cm、3+ 位 0.6cm，經 `DataCardModel.ParaFontSizeCm` 傳入），再以 `VerticalText.MatrixLayout` 於窗框內動態縮＋算下排起點，取代原本寫死 `baseFontCm=0.6` + 固定列距 2.6 的 `GroupFontPt`（單一往者因此放大到 0.8cm）；(3) **窗框「含整個框」右移 0.8cm**（`FrameShiftX=0.8`，框+故靈位+框內亡者名共用位移，相對位置不變）＋**預繳右移 1.5cm**（`txtPrepay Left 12.133→13.633`）。回歸鎖 `DataCard_SixDeadNames_MatrixStaysWithinMeasuredWindow`（改用 MatrixLayout 重算不超框）、`DataCard_number_is_number_only_title_separate`；待實體複驗。
>
> ✅ **2026-07-27 客訴：往者 1 位／2 位姓名在窗框中置中**（見 `DataCardRenderer.DeadColumnsX`）。1 位只用中欄、2 位用中欄＋右欄——這兩個欄位是為 3+ 位 2×3 矩陣設計的固定座標，落在框中軸右側，所以少人數時看起來沒置中。改為**依實際字級動態置中於框中軸 `FrameCenterX=17.278`**（＝「故」「靈位」所在線）：1 位左緣 `= 中軸 − fontCm/2`；2 位對稱分居中軸左右、中間留 `FewDeadColumnGap=0.1cm`（比照薦牌 2 位分支，順帶解掉 0.8cm 字級塞進 0.75cm 欄距會互相貼住的問題）。**框（矩形＋故／靈位）不動、3+ 位矩陣不動**。
>
> ⚠️ **踩雷（本輪二度客訴的根因）**：第一版做成「整組固定左移 0.3cm」（`FewDeadShiftX`，已移除）。固定位移量**不可能**同時對到 1 位與 2 位——兩者組寬不同（1 位＝`fontCm`、2 位＝`2×fontCm+0.1`），字級本身又有 0.8cm（1-2 位基準）／0.5cm（≥8 真字）／窗框內再等比縮三種可能；實測 0.3cm 讓 1 位偏左 0.19cm、2 位偏右 0.18cm，方向相反。**置中一律用字級回推**（同薦牌 `DeadCenterX − fontCm/2` 的做法，且必須在 `MatrixLayout` 算完字級之後才決定 X）。
>
> 「1 位／2 位」判定沿用 `PrintTemplateSelector.SlotTier` 的 **slot-based** 語意（`SlotTier` 因此由 private 改 public），與薦牌變體/字級判定同一套；**不可在此改用 Count 數總數**，名字填在後面欄位（有空洞）時會誤判——同一個誤實作正是 2026-07-18 文牒「往生者沒印」的客訴根因。200 DPI 回掃驗證（`reference/output/datacard_{one,two}_dead_centered_plain.pdf`）：框直線 15.761~18.796（中軸 17.278）／1 位墨跡中心 **17.266**／2 位整組中心 **17.272**，誤差 ≤0.012cm。回歸鎖 `DataCard_OneOrTwoDeadNames_AreCenteredInWindowFrame`（0.8/0.5 兩種字級 × 1 位／只填第 2 格／2 位，含空洞資料走矩陣）；110 Infrastructure 測試綠、待實體複驗。
>
> ✅ **2026-07-31 客訴：往者 2 位時左右相反**（見 `DataCardRenderer.DeadColumnsX` tier ≤2 分支）。07-27 置中改版把 d[0]（往者一）擺左、d[1]（往者二）擺右，方向錯誤——直書由右往左讀，**往者一必須在右、往者二在左**（薦牌 `TabletRenderer` 2 位分支本來就是 One 右／Two 左，資料卡才是不一致的那個）。改為 d[0] 左緣 `= 中軸 + 0.05`、d[1] 左緣 `= 中軸 − 0.05 − fontCm`；整組仍對稱置中於 `FrameCenterX=17.278`、中間仍留 `FewDeadColumnGap=0.1cm`，**字級、垂直位置、框、3+ 位矩陣、1 位置中全部不動**。注意 `DeadColumnsX` 回傳 tuple 的 `LeftX/CenterX/RightX` 是**矩陣欄位槽位**語意（Center→d[0]、Right→d[1]），不是幾何左右——此分支的 `CenterX` 在幾何上位於 `RightX` 右側。回歸鎖 `DataCard_OneOrTwoDeadNames_AreCenteredInWindowFrame` 補斷言 `CenterX > RightX`；81 Renderer 測試綠、待實體複驗。
>
...
| 屬性 | 值 |
|---|---|
| 紙張 | 21cm × 14.8cm（A5 橫），**白紙可印**（樣板 reference/template/普桌資料卡.jpg 僅量測/對位用） |
| 方向 | Landscape |
| 邊界 | 0cm |
| 字型 | 標楷體 |
| 特殊 | template（左葫蘆輪廓＋右側「電話：／備註：／確認無誤請簽名」標題＋簽名底線）**由程式全印**（2026-07-18 客訴改版，見下）；掃描 jpg 不進生產 PDF，僅 `debugOverlay` 對位用 |
...
- 右側 Phone（`Signup.Phone`）/ Remark（`Signup.Remark`）對齊樣板 label 上緣與冒號右緣（量測值見 positions §20），Remark 過長自動換行（不設 `.Height()` 不裁字）
- ✅ **2026-07-18 客訴改版：template 全印**（與資料卡同日同款客訴「普桌資料卡一樣也是要把 template 印出」）。`WorshipCardRenderer.DrawTemplate` 在生產路徑繪製：**葫蘆輪廓**重用 `worship2.png`（與卡片樣板同一款線稿，目視比對確認）以「墨跡落在樣板量測 bbox」反推圖框後 `FitUnproportionally` 縮放；右側三個標題（電話：／備註：0.6cm、確認無誤請簽名 0.7cm）＋簽名底線（x 13.6525 起、寬 4.9276 / y 13.792）依樣板墨跡量測繪製，繪製座標含渲染回掃校正（詳表見 positions §20）。校正與驗證用**生產字型**（`ReportFonts` 註冊真 BiauKai）渲染回掃，逐項誤差 ≤0.013cm；內容欄位座標完全未動。回歸鎖 `WorshipCard_EmptyContent_StillPrintsTemplate`（空內容也必須畫 template，防退回套印模式）
- **後端不限 SignupType**（2026-07-18 解鎖，原「限 type-4 丟 `WORSHIP_ONLY_TYPE_4`」已撤回）：任何型別都回 200 PDF，批次不過濾，對齊舊系統
- **入口二：新增報名表單的「列印資料卡」鈕自動選用（2026-08-12 使用者定案）**——該鈕依剛新增那筆的
  `SignupType` 分流：4（普桌）→ 本報表、其餘 → §1 資料卡，按鈕文字同步變「列印普桌資料卡」
- **前端右鍵 enable 條件（2026-08-15 使用者定案）**：選取列**全非普桌**時「列印普桌資料卡」停用
  （提示改印資料卡）、**全普桌**時反過來停用「列印資料卡」；**混選兩者皆可印**。
  只是前端引導，後端契約不變（見上一條）。列印預覽頁下拉不套此規則。
  ⚠ 兩張卡紙張同為 21×14.8cm 但驅動 form 名不同（「資料卡」／「普桌資料卡」），
  自動選紙不可互相替代（見 `PrinterFormMatcher`）——這正是要擋的理由。
  詳見 [business-rules-implicit §16.1 / §16.2](../business-rules-implicit.md)、[signup-management.md](signup-management.md)
...
- [ ] 普桌（牌位）不限 SignupType，選什麼印什麼（2026-07-18 解鎖，對齊舊系統）；後端 worshipcard 亦不限
- [ ] 右鍵兩張資料卡互斥停用：全普桌鎖「資料卡」、全非普桌鎖「普桌資料卡」、混選皆可（2026-08-15，§16.2）
- [ ] 多筆合併 PDF 順序與選取順序一致
