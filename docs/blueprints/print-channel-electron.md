...
>
> **2026-08-18 追加：自動選紙在這條路徑上的語意（現場問「選對話框時也會自動選紙嗎」）**
>
> **會**，但三件事都不一樣，而使用者只感覺得到第三件：
>
> | | 舊路徑（檢視器） | 決策 11（對話框） |
> |---|---|---|
> | 改哪一份 DEVMODE | **每使用者預設**（`SetPrinter`，全域、持久化） | **我們自己帶進對話框那份 copy**（不寫回系統） |
> | 什麼時候改 | 開預覽視窗**之前** | 按下列印**當下** |
> | 選不到紙時的回饋 | 寫進預覽視窗標題（`viewerTitle` 警語） | **原本什麼都沒有**——`formResult` 只進診斷紀錄 |
>
> 第三格就是「開著自動選紙卻好像沒作用」的來源：機制其實有跑，只是**選不到的時候沒人告訴使用者**，
> 而使用者能補救的時機（自己在對話框裡選紙）正好就是對話框開著的那幾秒。
> 處置：`printFormNoticeForDialog()` 在 **`dialog-shown` 當下**就把警語推到預覽頁，
> 並在成功訊息尾巴帶上「⚠ 但紙沒自動選到」（否則要等紙印歪了才知道）。
>
> 同輪修掉一個會讓自動選紙**靜默失效**的真 bug：`PRINTER_INFO_2.pDevMode`
> **允許是 NULL**（文件明說，有驅動就是不給），原本拿不到就直接放棄 ⇒ 對話框收不到我們的 DEVMODE，
> 於是①自動選紙沒作用、②**整條路退回「對話框自己去讀每使用者預設」——正是決策 11 要繞開的那一條**。
> 現在退而取 `DocumentProperties` 那一份（仍是我們手上的 copy，不寫回系統，不變式不受影響），
> 並把來源與失敗原因寫進診斷紀錄（`devmodeSource: "printer→user"` / `devmodeResult`）。
>
> **可轉移教訓**：同一個功能在兩條路徑上「行為等價」不代表「使用者感受等價」——
> 搬路徑時要一起搬的還有**失敗時的回饋**，否則等於把功能做成薛丁格的。
