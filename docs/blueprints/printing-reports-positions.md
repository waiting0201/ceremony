...
> 1. **編號往下 0.1cm**：Number `Top 0.1 → 0.2`（Left 維持 0.5；9 變體共用；OneOne 2cm Margin 補償不變）。
...
> 4. **陽上 1、2 位右移 0.5cm**：1 位陽上 `LivingNameOne Left 0.83528 → 1.33528`（OneOne/TwoOne/UnderscoreOne 共用）；2 位陽上 `l[0] 1.2825 → 1.7825`、`l[1] 0.30611 → 0.80611`（OneTwo/TwoTwo/UnderscoreTwo 共用）。附帶緩解 l[1] 原逼近印表機不可列印邊界的風險。3-6 位陽上矩陣**不動**。
