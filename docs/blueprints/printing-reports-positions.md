...
> - ⚠️ **資料卡連動**（`ReportModelBuilders.DataCard` 取同一個 `ParaFontSize`）：2 位往者 7 字素的案例會一併從 0.8 → 0.6cm，屬 07-21「資料卡往者字級與薦牌一致」既有決策下的預期行為。但資料卡端**仍保留** `MatrixLayout` 的動態縮放、`FewDeadColumnGap` 也維持 0.1（窗框較窄，使用者只指定薦牌）——**刻意的範圍差異**。
>
