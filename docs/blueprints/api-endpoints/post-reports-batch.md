...
last_updated: 2026-07-04 (reportType 白名單加 worshipcard（普桌資料卡）；SignupType=4 防呆條件泛化為 worship or worshipcard。先前：加 signupIds[] 精準勾選列印模式，取代原本「多筆勾選只能近似成編號區間」的 v1 限制)
---
...
| reportType=worshipcard（普桌資料卡，2026-07-04 新增） | 無舊系統對應 | SignupType=4 防呆與 worship 完全一致（上兩列規則同樣適用；程式碼是同一個 `reportType is "worship" or "worshipcard"` 分支） | 沿用 worship 測試覆蓋同一分支 |
