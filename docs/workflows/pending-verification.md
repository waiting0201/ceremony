...
| B12 | **決策 11：按下「列印」後對話框會正常關閉**（STA 修正） | v2.5.1 | 對話框路徑印一次：按下列印後**對話框要自己消失**、預覽視窗要能操作。這是 2026-08-18「對話框卡住關不掉、換印表機也一樣」的修正驗證 —— **任何一台印表機都驗得到，不必等 PA2000** |
...
>
> B12 **要先做**：它若沒過，B9／B11 整場都做不下去（按了列印就卡住）。B11 併進 B9（決策 11 對照組）那一場做——它本身就是為了讓 B9 那場出事時**看得到是哪一種失敗**。
