...
last_updated: 2026-08-18 (**承認現場回報頻寬只有一句話**〔客戶只會回「可以了」／「還是不行」，要不到訊息內容或 log〕：新增該段原則；B10 以客訴消失**間接**結案並標明證據等級、B11／B13 降級為「不靠人驗」〈細節本來就寫在 print-*.log；止血鍵是下次故障才會用到〉、B9 改成一句話問法。同日先前 (**B12 現場確認通過**〔客戶回報「印表機可以了」⇒ STA 就是根因，決策 11 送印路徑第一次真的印出東西；B9 六報表對照組因此解除阻塞〕。同日先前 (新增 B12／B13：決策 11 的 STA 修正〈按下列印後對話框卡住關不掉、換印表機也一樣〉與「中止列印視窗」止血鍵；**B12 要先做**，它沒過就整場做不下去。同日先前 (新增 B11：決策 11 送印結果事後回報〈PA2000「按了列印沒有反應」客訴的產物——對話框之後的四種結局原本畫面上完全無法區分〉。同日先前 (新增 B10：v2.5.0 關掉預覽視窗會跳主行程錯誤框（closed handler 讀已銷毀的 webContents），修正後需實機確認錯誤框消失且清理鏈有跑到。先前 2026-08-17 (建檔。起因：v2.5.0 發版時發現「待複驗」項目已從 v2.4.5 一路順延**六個版本**，因為它們只存在於 status.md 各版 Recently Done 條目的內文裡——沒有單一清單、沒人知道總共欠幾項。本檔把它們集中，並附上在 HEAD 重產的樣張路徑，讓一次實體套印就能全部掃完。))))))
---
