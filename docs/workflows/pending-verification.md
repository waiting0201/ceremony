...
last_updated: 2026-08-21 (新增 B15：報名維護右鍵「移動插入至…」〈移位並讓中間遞補；code 與真實 MSSQL 整合測試都綠，但沒被真人在現場資料上按過〉。先前 2026-08-18 (新增 B14：對話框路徑的自動選紙〈機制本來就有跑，但選不到時原本完全不出聲；v2.5.2 起會在對話框開著的當下講〉。同日先前 (**承認現場回報頻寬只有一句話**〔客戶只會回「可以了」／「還是不行」，要不到訊息內容或 log〕：新增該段原則；B10 以客訴消失**間接**結案並標明證據等級、B11／B13 降級為「不靠人驗」〈細節本來就寫在 print-*.log；止血鍵是下次故障才會用到〉、B9 改成一句話問法。同日先前 (**B12 現場確認通過**〔客戶回報「印表機可以了」⇒ STA 就是根因，決策 11 送印路徑第一次真的印出東西；B9 六報表對照組因此解除阻塞〕。同日先前 (新增 B12／B13：決策 11 的 STA 修正〈按下列印後對話框卡住關不掉、換印表機也一樣〉與「中止列印視窗」止血鍵；**B12 要先做**，它沒過就整場做不下去。同日先前 (新增 B11：決策 11 送印結果事後回報〈PA2000「按了列印沒有反應」客訴的產物——對話框之後的四種結局原本畫面上完全無法區分〉。同日先前 (新增 B10：v2.5.0 關掉預覽視窗會跳主行程錯誤框（closed handler 讀已銷毀的 webContents），修正後需實機確認錯誤框消失且清理鏈有跑到。先前 2026-08-17 (建檔。起因：v2.5.0 發版時發現「待複驗」項目已從 v2.4.5 一路順延**六個版本**，因為它們只存在於 status.md 各版 Recently Done 條目的內文裡——沒有單一清單、沒人知道總共欠幾項。本檔把它們集中，並附上在 HEAD 重產的樣張路徑，讓一次實體套印就能全部掃完。))))))))
---
...
2026-08-18 整輪的實測結論：客戶能給的回覆是「印表機可以了」「還是不行」這種等級，**要不到「訊息長什麼樣」「log 貼一下」「佇列裡有沒有那筆工作」**。這不是配合度問題，是現實。

因此本段每一項的設計規則：

1. **驗收項目要能用一句話回答**（「印出來的位置跟以前一樣嗎？」可以；「請看那行訊息寫什麼」不行）
2. **需要細節的東西一律寫進檔案**，不要靠問——`print-*.log` 的 `printResult` / `win32` / `jobId` 就是為此存在；要細節時我們自己去讀那個檔
3. **驗不到就標降級（⤵️），不要當成通過**；以「客訴消失」結案的要標成「間接（✅）」並寫明證據等級是「沒人再抱怨」
...
| B10 | ✅（間接）關掉預覽視窗**不再跳主行程錯誤框** | v2.5.1 | **2026-08-18 以客訴消失間接結案**：那個 Error 對話框是客戶自己回報的症狀（`reference/print_error.png`），現在回覆「沒問題了」⇒ 症狀不在了。⚠️ 證據等級是「沒人再抱怨」，不是「有人確認過」——若日後又出現，回頭看 `closed` handler |
| B11 | ⤵️ **降級：不再要求現場回報** | v2.5.1 | 現場回報頻寬只有一句話（見下），所以這一項**不靠人驗**：同一份資訊（`printResult` / `win32` / `jobId`）本來就會寫進 `%APPDATA%\Ceremony\logs\print-*.log`，下次真的出事時去讀那個檔即可。畫面上那行訊息是給使用者當下看的，不是給我們驗收的 |
...
| B13 | ⤵️ **降級：故障時才會被用到** | v2.5.1 | 這顆鍵是**下一次卡住時**的逃生口，平常沒人會按也不需要按。不主動要求驗；真的再卡住時請客戶按它（比關掉整個程式溫和），紀錄裡會留下 `print-dialog-aborted` |
| B14 | **對話框路徑的自動選紙**（有沒有選到 + 選不到會不會講） | v2.5.2 | 一句話問法：「**開著自動選紙印一張，列印視窗裡的紙張有沒有自動變成『資料卡』？**」。沒變成的話，預覽頁上應該會出現一行黃底警語說明原因（例：這台印表機沒有「資料卡」紙張設定）——**有那行就不是 bug，是驅動裡沒建那張表單** |
| B15 | **報名維護右鍵「移動插入至…」** | v2.5.3 | 一句話問法：「**選一筆報名按右鍵『移動插入至…』，輸入另一個編號，中間的編號有沒有自動接上（沒有空號）？**」。順手看兩件事：目標編號輸入超過最後一號時應跳「目標編號超出範圍（目前 N–M）」而**資料完全不動**；同年同法會的**其他報名類型**編號不該被連帶動到 |
...
5. B9 最後做（量最大，且結果決定 `printViaDialog` 要不要翻預設）。**一句話版本的問法**：「用『對話框』印出來的位置，跟你以前印的**一樣嗎**？」——六張都問這一句，比要求逐張比對現實得多
