...
### 1.6 編號重排的兩條路徑（新版增強，legacy 無）

舊系統只能「自動 MAX+1」或「手動指定空號」，指定已佔用號一律被 `編號重複` 擋下。新版另有兩條會**動到別人編號**的路徑：

| 路徑 | 入口 | 語意 | 總筆數 |
|---|---|---|---|
| 插入並順移（2026-07-04） | 右鍵「在此前插入」→ `POST /signups/insert-shift` | 於 N 插入**新的一筆**，同群組 `Number ≥ N` 全部 +1 | +1 |
| 移動並重排（2026-08-21） | 右鍵「移動插入至…」→ `POST /signups/{id}/move-number` | 把**既有一筆**移到 N，中間區段 ±1 讓位 | 不變 |

移動的語意（兩個方向對稱，＝「該筆佔據目標編號，中間區段整體讓位」）：
...
1. **目標超出該群組現有 `MIN..MAX` → 擋下**（訊息帶「目前 N–M」），**不 clamp 成最後一號**。
   照字面搬會在中間留一段空號，與「移位就是為了不留空號」的需求正好相反。
2. **完全不寫 SignupLog**——移動那筆與被讓位的列都不 append（同「在此前插入」順移列的取捨：
   `SignupLogs.Number` 是歷史快照、且避免一次操作在大群組灌進上百筆 log）。
   **代價明寫在此以免日後誤判為 bug：在「瀏覽歷程」看不出某筆被移過。**
3. **只做單筆**（多筆連續移動語意複雜且現場無需求）。

兩條路徑共通：**刻意不做編號重複檢查**（目標位置本來就被佔用，那正是要讓位的對象）；
只在同 `(Year, CeremonyCategoryID, SignupType)` 內作用；並發安全靠 `signup-number:` applock +
`UPDLOCK/HOLDLOCK`（見 [backend-design.md](design/backend-design.md)「報名編號的群組互斥鎖」）。
與 §1.2 的唯一性檢查不衝突——那條管的是**編輯單筆改編號**（`PUT /signups`，仍以 `SIGNUP_NUMBER_CONFLICT` 擋重複）。
...
> ⚠ **2026-08-15 起本條有一項前端例外**：右鍵的**兩張資料卡**（一般 / 普桌）改為依報名類型互斥停用，見 §16.2。
> 本條其餘部分（「列印普桌」牌位、收據、薦牌、文牒、後端全部契約、列印預覽頁下拉）維持不變。

- SignupForm 右鍵 menu「列印普桌」：**與其他列印選項完全一致——有選取列即 enabled、選什麼印什麼**，前後端都不檢查 `SignupType`（「列印普桌資料卡」的後端亦同，僅前端 enable 條件見 §16.2）
...
> **適用範圍限「使用者明示選擇」的入口（2026-08-12 釐清、2026-08-15 修訂）**：本條管的是列表右鍵 menu
> 與列印預覽頁下拉——使用者自己指定了要哪一種報表，系統就照印、不得代為否決。
> **兩個例外，都出於「兩張資料卡同尺寸、印錯不易察覺」**：
> - 新增報名表單的「列印資料卡」鈕沒有給使用者選卡別，屬**系統代選** → 依類型自動分流（§16.1）
> - 列表右鍵的兩張資料卡雖是明示選擇，但錯的那一張**必定印錯紙** → 互斥停用（§16.2）
>
> 三者共存的原則：「別擋使用者的明示選擇」仍是預設，只有在**選錯必錯、且永遠有正確的另一項可選**時才擋。
...
> 4. 2026-08-15 只把**兩張資料卡**改回互斥停用（§16.2）；牌位類（列印普桌）與後端維持第 3 步的全面解鎖
...
### 16.2 報名維護右鍵：兩張資料卡依報名類型互斥停用（2026-08-15 使用者定案，**新版加值規則**）

§16.1 的同一套判斷延伸到列表右鍵。依**選取列集合**（右鍵未選取的列時＝該列）決定：

| 選取內容 | 列印資料卡 | 列印普桌資料卡 |
|---|---|---|
| 全部都是普桌（type 4） | 停用「普桌報名請改印『普桌資料卡』」 | 啟用 |
| 全部都不是普桌 | 啟用 | 停用「非普桌報名請改印『資料卡』」 |
| **混選**（普桌＋非普桌） | **啟用** | **啟用** |
| 未選取任何列 | 停用「請先選擇報名資料」 | 同左 |

- **為什麼只擋這兩項**：兩者紙張同為 21×14.8cm、外觀相近，印錯不易當場察覺，但驅動 form 名不同，
  自動選紙不可互相替代（同 §16.1 的 ⚠）。收據 / 薦牌 / 文牒 / **列印普桌（牌位）**一律不受影響。
- **為什麼不算重蹈 2026-07-18 覆轍**：當年是**單向鎖**（非普桌時普桌類全灰），使用者會走到「什麼都印不了」
  的死路才引發客訴；這版是**互斥**，任何選取狀態下永遠至少有一張資料卡可印，且停用項都有 tooltip 說明改印哪張。
- **混選刻意不擋**：跨型別批次列印是使用者明知的操作，回到 §16「別擋明示選擇」的立場。
  故實作條件用 `every` / `!some`（signup-list-page.ts `dataCardGate`），不是 `some` / `!every`。
- **範圍**：只有報名維護的右鍵選單（含共用同一份 menu 的工具列「對選取項目」kebab）。
  **列印預覽頁的報表下拉不套**（2026-08-15 使用者定案：那是使用者刻意進去挑報表的地方）。
- **後端完全不變**：`GET /reports/datacard|worshipcard` 仍不檢查 `SignupType`，任何型別都回 200，
  批次亦不過濾——這是純前端的引導，不是契約限制。
- **舊系統無此規則**（舊系統沒有「普桌資料卡」這張報表），與 §16.1 同為新版加值。
