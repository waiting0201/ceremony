...
- [x] **GitHub Actions Windows 打包 workflow** — Done 2026-06-18
  - 新增 [.github/workflows/release.yml](../.github/workflows/release.yml)：`v*` tag / 手動觸發 → `windows-latest` 跑 `setup-dotnet`（讀 global.json 10.0.103）+ Node 22 → `npm ci` → `pwsh backend/publish.ps1` → `npm run electron:build` → `npx electron-builder --win --publish never` → upload-artifact + tag 時 `action-gh-release` 附 `setup.exe`/`.blockmap`/`latest.yml`
  - 解決「mac 無法產 NSIS 安裝檔」：CI windows runner 打正式 exe。**刻意不跑 `npm run dist`**（bash 寫法 Windows 會炸）；簽章未配置（自動跳過）。doc 同步 [infrastructure.md](design/infrastructure.md) CI/CD 段

- [x] **合併遠端 `windows打包` 分支** — Done 2026-06-18
  - 兩邊各自實作「打包預設連線」做法相反；依使用者決定保留本地硬編 `DEFAULT_CONFIG`、丟棄遠端種子檔機制，吸收遠端獨立工作（sidecar cwd 修正、無選單列、installer.nsh、backup-page 直寫 D:\Backup）。merge commit `f0cbcf8`（雙 parent）
...
- [x] **2 個新 endpoint shipped (GET /categories + GET /signups) + PredicateBuilder→Dapper 動態 SQL** — Done 2026-05-27
  ...
  - **Secret 驗證**：`grep -rE "<dev-sa-pwd>|<prod-sa-pwd>" backend/` 仍 0 命中 ✅
