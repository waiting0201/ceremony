...
last_updated: 2026-07-31 (薦牌列印客訴：往者整體左移 0.05cm——排版中心線 DeadCenterX 5.785→5.735，係 07-21「右移 0.1cm」的回收微調〔相對量測基準 5.685 淨右移 0.05〕；9 變體的 1 位置中／2 位左右分居／3+ 位 2×3 矩陣共用這一個常數，欄間相對關係與 Top／字級／陽上／其他報表皆不動；200DPI 回掃三欄墨跡中心 4.794/5.721/6.661、2 位對稱中心 5.734、橫向範圍 4.521~6.934 仍在窗框內緣 4.191~7.163 內；既有測試不鎖 X 故無須改斷言，Infrastructure 113／Application 222 綠、待實體套印複驗。先前同日 新增報名表單客訴第四輪 5 項：①地址欄位標題全拿掉、寄件/文牒**各一個 fieldset 外框**、「同寄件地址」勾選移進文牒框內〔部分反轉 07-29 的「全部拿掉外框」〕②往生/陽上名單各自恢復外框與 legend，解掉「填字後失去標示」取捨 ③基本資料改序 堂號→姓名→電話→員工類型→固定編號、前三欄以「與 .form-cols 同構」的巢狀達成與地址框逐像素等寬 ④勾了「指定編號」不再被改選信眾清掉〔勾選與數字都留；與費用同一取捨，也回到舊 BelieverSelected 未碰 cbKeepNumber 的行為，resetBelow 仍清〕⑤「同寄件地址」放寬為「城市/區域/地址三者全空才擋」〔刻意偏離舊系統〕；信眾表單同步 ①⑤；順帶發現 believer-edit-form 的 mailAddress 仍 required、與 07-21 放寬不一致，未動列 B15；ng build 0 warning、ng test 31 綠〔新增 3 案回歸鎖〕，版面待實機複驗。先前 2026-07-29 (發版 v2.3.5〔PATCH，資料正確性修復＋DbUp 0004 DML migration，正式機由 sidecar 啟動自動套〕；同日先前修復郵遞區號沒寫進 DB 的客訴：Signups 的郵遞區號有 MailZipcodeID〔FK〕與 MailZipcode〔文字快照〕兩欄，SignupView 曝的是**文字欄**，新版三個寫入點只寫 FK → 新系統建的報名郵遞區號全空、UPDATE 漏寫還會留舊號碼印錯；改由 FK 現查同步寫入 + DbUp 0004 回填 164 筆空值〔文字與 FK 不一致的 277/275 筆刻意不動，列 B14 待業務確認〕；新增整合測試回歸鎖，後端 222/113/75 全綠。同日先前發版 v2.3.4〔PATCH，自 v2.3.3 起累積：互動字級 +1px、Electron 啟動最大化、報名表單版面客訴第三輪、報名維護清單客訴 3 項——全區間皆 UI 樣式/版面/預設值，無契約與資料結構變更；4 項均待實機複驗〕；同日先前報名維護清單客訴 3 項：法會欄預設寬 100→64px〔＝三個字〕、「顯示完整表格」取消 localStorage 記憶〔每次開頁不勾，欄寬持久化不動〕、「全部」語意收斂為只鎖年份/法會/類型〔＋年份修飾條件「範圍」〕其餘條件仍可搜尋——buildQuery 由短路回空物件改成只把這三欄送 null/-1；ng build 綠、ng test 28 綠，待實機複驗；同日先前新增/修改報名版面客訴第三輪 4 項：基本資料排成一整列並上提為全寬列〔右欄編號/費用/備註/預繳隨之下移〕、按鈕列靠左＋「確認」套新的全域 .btn-wide〔min-width 112px＝兩字鈕自然寬 ×2〕、五個區塊（基本資料/地址/往生名單/陽上名單/編號·費用·備註·預繳）全部拿掉 fieldset 外框與 legend 改 .bare-block〔只剩「法會資料」保留外框；名單填字後失去往生/陽上標示屬已知取捨〕、「新增報名成功」提示改 ConfirmDialog 新的 emphasis 旗標〔訊息 20px + 確定鈕加寬〕而非再動全站 dialog 字級；ng build 綠、ng test 28 綠，待實機複驗；同日先前 Electron 視窗啟動即最大化：ready-to-show 內 maximize() 再 show()，width/height 留作還原尺寸，不用 fullscreen 以保留標題列；同日先前：輸入欄位/搜尋結果清單/按鈕字級 +1px、標題不動：另開 --font-size-*-plus 平行 token 避免調基礎級距連標題一起放大，套用全域 .btn/.data-table/vgrid/.field/.dense-controls ＋ 報名維護 vgrid、兩張表單的名單欄/搜尋框/結果表、登入頁、報表 tab、setup 頁；清單列高不動，ng build 0 warning，待實機複驗；先前 2026-07-28：發版 v2.3.3〔PATCH，自 v2.3.2 起累積：批次列印進度 overlay ＋ 收據郵寄封面客訴 ＋ 報名彈窗/搜尋 UI 調整；新增 4 個 batch job endpoint 照字面屬 MINOR，使用者指定用 PATCH——新端點是既有 POST /reports/batch 的內部替代路徑、同步版契約完全保留，故不算對外新功能面〕；同日先前批次列印進度 overlay：改 job 模型〔POST /reports/batch/jobs + GET 進度 + GET file + DELETE 取消〕，UI 顯示真實 i/N 百分比與取消鈕，完成即消失；驗證/查詢仍同步故錯誤碼不變；否決 SSE 改 250ms 輪詢（EventSource 帶不了 Authorization、token 進 query 會被 Serilog 記錄）；順手修 CORS 缺 WithExposedHeaders 的既有 bug；Application 222／Integration 74／Infrastructure 113／前端 28 全綠、兩端 build 0 warning、Playwright 端對端實測進度與取消；發現 PdfSharp 合併 2GB 上限（19018 筆會爆，屬既有限制，未處理）。同日先前收據郵寄封面客訴兩項：姓名下移對齊預印「大德　法啟」列 Top 5.44111→6.13（客戶確認只下移、左緣不動；錨值取自實印信封回掃 reference/template/收據封面.jpg 200DPI 量測）＋地址過長自動換行不壓姓名列（新增 CoverAddressFontPt/CoverAddressLineCount 字級降階，一般地址維持 16pt），＋封面頁 debugOverlay 補上（客戶回掃圖裁成 1441×665 灰階 PNG 進 EmbeddedResource、Left 0.05/Top 0/20.93×9.66cm，疊圖回掃姓名對預印「大德」中心差 0.000cm；QuestPDF Image 負位移／超出頁寬會整張靜默消失入 gotchas），回歸鎖 Receipt_CoverName_SitsOnPrePrintedDaDeRow_AndAddressWrapsAboveIt、Infrastructure 113 綠、待客戶實印複驗；同日先前報名維護搜尋範圍 5 個 checkbox（姓名/陽上/往生/電話/備註）預設改全勾、關鍵字欄開頁即可輸入（刻意偏離舊系統全不勾）；同日先前新增/編輯報名 overlay 改成點 backdrop 與按 Esc 都不關（form-overlay 新增 dismissible input，報名維護給 false），只剩 × 與表單的取消鈕；ng build 綠，待實機複驗；同日先前新增報名草稿改一律快照——離開時無條件存當下畫面、拿掉 dirty 門檻與存檔成功/取消的 clear()，補上「畫面看得到卻帶不回來」的兩個破口，草稿加存 lastCreatedSignupId 與 dirty；同日先前按「取消」不再清空費用（resetBelow 拿掉 fee: null，保留範圍＝法會資料＋費用，與「改選信眾不清費用」同一取捨，回歸鎖 1 案）；同日先前 Enter 不送出（拿掉隱藏 submit 鈕＋ngSubmit、form 層攔 Enter、textarea 放行，回歸鎖 1 案，踩雷入 gotchas）＋成功提示 dialog 內文放大到 --font-size-md（＝側欄選單字級，改共用 .confirm-body，全站 dialog 一起）；同日先前版面客訴第二輪：「同寄件地址」改夾在寄件段與文牒段中間且靠右、文牒郵遞區號回到文牒區域右邊、文牒地址加寬佔滿三欄、「列印資料卡/取消/確認」移到備註下方〔表單新增 form-actions 投影 slot、form-overlay 新增 showActions input 收掉 footer、路由頁 .form-actions 移除〕；同日先前四項：信眾搜尋框字級對齊地址欄〔.search-input 不在 .field 內、只繼承 font-family，字級落回 UA 預設 13.33px〕、往生/陽上名單由右欄移到左欄地址下方、「同寄件地址」移到文牒郵遞區號正上方〔第一列第三欄，郵遞區號下移到文牒地址同列，不多佔列高〕、備註 rows=1→4；ng build 綠，待實機複驗；先前 2026-07-27 (發版 v2.3.2〔PATCH，累積列印客訴修正與報名維護 UI 調整〕；同日文牒列印客訴兩項：①往者/陽上「不管幾位字級都跟一位一樣」——改用 VerticalText.MatrixLayout（同薦牌 07-17），字級固定 0.8cm、下排起點動態＝上排最長字數+1 個字高，撤掉 07-21 的 WithBottomGap＋固定列距（那組合把 3 字名縮到 ≈0.50cm）②往者最左欄離左側預印字 0.5cm 是**間距**不是位移——撤掉 07-21 誤解的 DeadShiftX=0.5（實際間距變 0.793cm），改絕對錨點 DeadLeftX=11.707（量測「鳴呼既追攀…」欄右緣 11.207+0.5），Base/Two 兩變體皆以自己的最左欄對齊、欄間相對距離維持 RDLC；堂號 Second 同錨、First 維持相對 2.03753；200DPI 回掃最左欄墨跡 11.756／堂號 11.769（差 0.013cm）、6 位滿版字高 0.685cm 未超框；回歸鎖 Text_NameFontSize_StaysAtBase_RegardlessOfNameCount／Text_DeadNamesAndHallName_KeepHalfCentimeterFromPrePrintedText，Application 207／Infrastructure 112／Integration 67 全綠、待實體套印複驗；同日先前資料卡列印客訴兩輪「往者 1／2 位沒置中」：第一輪固定左移 0.3cm 無效〔1 位偏左 0.19、2 位偏右 0.18，方向相反〕，改為依實際字級動態置中於框中軸 FrameCenterX=17.278（1 位 中軸−fontCm/2、2 位對稱分居留 FewDeadColumnGap 0.1cm，X 在 MatrixLayout 算完字級後才決定），框與 3+ 位矩陣不動；1/2 位判定共用 PrintTemplateSelector.SlotTier〔private→public，維持 slot-based 不可改 Count〕、三欄 X 抽 internal DeadColumnsX 供測試斷言；200 DPI 回掃 1 位中心 17.266／2 位 17.272（誤差 ≤0.012cm），回歸鎖 DataCard_OneOrTwoDeadNames_AreCenteredInWindowFrame，Application 207／Infrastructure 110／Integration 67 全綠、待實體套印複驗；同日先前報名維護列選取補齊 shift 範圍選取：列首 checkbox 從 (change) 改綁 (click)〔change 事件不帶 shiftKey，原本從 checkbox 點永遠吃不到 shift〕、範圍改錨點式重算故可縮小且不吃掉更早的選取、mousedown 擋 shift 文字反白；新增 signup-list-page.spec.ts 7 案，ng test 19 綠；同日先前報名維護搜尋面板新增「全部」checkbox（置於「範圍」上方）：勾選＝忽略所有條件顯示全部報名供比對、條件值保留但停用變灰，取消勾選即以原條件重查還原；buildQuery 單點短路讓匯出 Excel 跟著模式走；isAll+searchedBeforeAll 併入 SignupSearchState 跨路由保存；後端零改動（空條件即 WHERE 1=1）；沿用三列 grid 不加高，col 1 變 全部/範圍/顯示完整表格；ng build 0 warning、ng test 12 綠；同日先前補回新增報名「列印資料卡」按鈕（存檔前 disabled、成功後啟用，對齊舊 btnPrintDataCard）＋報名維護 overlay 固定 1100px＝對齊新增報名頁寬度（form-overlay 新增 width input；限制內層表單無效會讓 actions 落單）；同日版面續調三項：重複報名警示移到左欄法會資料下方、編號 disabled 補全域灰底樣式（.field 原本蓋掉瀏覽器預設 disabled 外觀）、信眾結果高度改表頭+4 列；同日先前新增報名版面三項：法會資料改左側直立窄欄（回舊 plStep1）、信眾搜尋結果縮為表頭+3 列、編號欄恆顯示改 disabled/enabled（回舊 cbKeepNumber_CheckedChanged）；同日往生/陽上名單 input 文字色對齊地址欄（.field 外吃不到全域 color，落回 UA fieldtext 純黑），報名+信眾兩表單同步；同日改選信眾不再清空費用（使用者輸入的金額保留，對齊舊 BelieverSelected 未碰 txtFee）；同日新增報名成功後不清表單/不關閉、跳「編號X，新增報名成功」單按鈕 dialog、草稿記憶清掉（對齊舊 NewSignupForm:355-361）；同日新增報名跨路由草稿：填到一半切到其他功能頁再回來資料保留——SignupDraftState root singleton，僅純新增模式、僅記憶體、靜默還原，儲存成功/按取消才作廢，純新增模式關閉 overlay 不再跳未儲存確認；同日追加改選信眾 in-flight race guard〔pickToken/isStale，快速改選不再互相污染〕＋選信眾即標髒〔沒打字也存草稿〕；ng test 7 綠、ng build 0 warning，待實機複驗；同日先前修掉 NU1903 Microsoft.OpenApi 2.0.0 高風險弱點：直接 pin 2.7.5 覆寫 transitive（升 Microsoft.AspNetCore.OpenApi 無效，上游 nuspec 下限仍 2.0.0），build 0 warning、vulnerable 清單全空、/openapi/v1.json 200 實測、三套測試全綠；同日先前信眾搜尋補回舊 BelieverView 語意：GET /believers 加 searchKey 參數（14 欄 OR），前端併查 /signups + /believers?searchKey= 讓「從未報名過的信眾」也搜得到（補列排在報名列後、獨立額度 50 列），Application 207／Integration 67 綠；同日先前新增報名「選信眾帶錯資料」客訴修正：信眾搜尋結果一列＝一筆報名，點選改帶「該筆報名」的姓名/電話/兩組地址/往生+陽上名單/備註，信眾主檔 GET /believers/{id} 降為欄位為空時的 fallback 與摘要卡資料源，對齊舊 BelieverSelected:991-1101；年份/法會/類型/編號/費用仍不帶、費用清空、預繳續走 prefillPrepayHistory；ng build 綠，docs 同步 signup-management/legacy-coverage new-signup-form rows 4/34，待實機複驗；先前 2026-07-21 文牒列印客訴五項：①往者/陽上字級統一 0.9→0.8cm（NameBaseFontCm）②同欄上下姓名間空一個全形（沿用 VerticalText.WithBottomGap，往者 d[0]/d[5]·d[1]/d[3]·d[2]/d[4] 與陽上 lv 三組同欄配對，不動座標）③往者整體右移 0.5cm（DeadShiftX，Base/Two 兩變體所有 DeadName Left）④陽上整體下移 1cm＋右移 1cm（LivingShiftY/LivingShiftX，全部 LivingName）⑤地址 PhotoAddress 右移 0.5cm（bandRight 25.4+0.75→25.9+0.75）＋PNG canvas ×Supersample=4 提高解析度（27→108px/字·653→2612px·字級 27→108，帶尺寸 0.75×18.13cm 不變 ≈366DPI；可容字數/兩欄折行比例等比不變）；TextRenderer/SkiaImageHelpers 實作、Infrastructure 109 測試綠（Skia_VerticalAddress_* 以常數斷言隨 Supersample 等比通過）、text_mail1_sample/text_six 疊圖目視上下排全形間距明顯，docs 同步 positions §12/§13，待客戶實體套印複驗；同日先前資料卡列印客訴三項：①編號抬頭與號碼分開繪製、中間留固定 0.3cm 空隙、不再用「.」（ReportModelBuilders.DataCard 改傳 NumberTitle+Number 兩欄；DrawNumberField 用 QuestPDF Row+Spacing(0.3cm) 依抬頭實際渲染寬度定位號碼——半形 "No" 與全形 "信" 空隙皆準，不可用「字數×1.0cm」估算否則半形抬頭空隙過寬）②往者字級邏輯改與薦牌一致（起點 ChooseTablet 的 ParaFontSize tier 經 DataCardModel.ParaFontSizeCm 傳入 + VerticalText.MatrixLayout 於窗框動態縮＋算下排起點，取代寫死 0.6cm+固定列距 2.6 的 GroupFontPt；單一往者放大到 0.8cm）③窗框「含整個框」右移 0.8cm（FrameShiftX，框+故靈位+框內亡者名共用位移）＋預繳右移 1.5cm（txtPrepay Left 12.133→13.633）；DataCardData/DataCardModel 加 NumberTitle+ParaFontSizeCm 欄、DataCardNumber helper 移除、回歸鎖 DataCard_SixDeadNames_MatrixStaysWithinMeasuredWindow 改用 MatrixLayout 重算＋ReportNumberFormatTests 改 DataCard_number_is_number_only_title_separate，Application 205＋Infrastructure 76 全綠、疊圖目視 OK，docs 同步 printing-reports/printing-reports-positions §1，待客戶實體套印複驗；同日先前薦牌列印客訴四項微調：①編號往下 0.1cm（Number Top 0.1→0.2）②往者 1/2 位且 ≥8 真字字級 0.6→0.5cm（PrintTemplateSelector 長名分支；3+ 位仍 0.6）③往者整體右移 0.1cm（DeadCenterX 5.685→5.785，1/2/3+ 位共用中心線）④陽上 1/2 位右移 0.5cm（LivingNameOne Left+0.5、2 位 l[0]/l[1] 各+0.5，附帶緩解 l[1] 逼近不可列印邊界）；PrintTemplateSelectorTests 三案例 0.6→0.5cm、Application 28＋Infrastructure Tablet 17 全綠，docs 同步 printing-reports-positions §3，待客戶實體套印複驗；同日先前：報名維護 UI 客訴四項：編號＋批次列印起迄加 .num-stepper ▲▼ ±1、編輯表單預繳民國年移下一行置於預繳法會前、搜尋/列印按鈕補 align-self:stretch 撐滿列高、清單垂直捲軸右鍵子選單〔捲到這裡/頂端/底部/上下頁/上下捲〕，ng build 綠，docs 同步 frontend-design/visual-design/signup-management；同日先前：客戶端 migration 定案方案 B：Api sidecar 啟動自動跑 DbUp——MigrationRunner 抽出、Api ProjectReference Ceremony.Migrations 隨 publish 打包、Program.cs Build 後 Run 前執行、fail-fast、config Migration:RunOnStartup 可關、整合測試工廠設 false、實機 dotnet run 驗證「No new scripts→0 支」啟動；docs 同步 data-migration/infrastructure/security/electron-packaging；同日員工類型/固定編號/堂號改 per-signup 可編輯（方案 A，反轉 2026-06-29 方案 C）：新建 Ceremony.Migrations DbUp 專案（0001 Signups 加三欄 / 0002 回填凍結 / 0003 SignupView 改 COALESCE＋新增數值 EmployeeType），後端 CreateSignupRequest+SignupWriteModel+SignupListItem 加欄、三 handler 寫 Signups 自有欄不回寫 Believer、Prepay 編譯修，前端表單三欄改可編輯（select/checkbox/input）＋pick/apply/submit/resetBelow/models 同步，預繳保號仍讀信眾；migration 已套本機 DB 驗證，Application 205／Integration 66（含 per-signup 隔離＋COALESCE 回退）／Infrastructure 107 綠、ng build 綠；docs 同步 signup-hallname-isolation（C→A）/database-design/post-signups/glossary/business-rules §3.1/signup-management/legacy-coverage/pending B13；同日稍早新增報名頁客訴六項：①「取消」＝清成新的一筆不關閉/跳頁（resetBelow，保留法會資料、清信眾與其以下全部欄位）②改選/重搜信眾殘留欄位修復（pickBeliever 覆蓋前先清費用/備註/預繳，預繳先歸零再條件回填）③搜尋結果 list 文字＝地址（.believer-results 覆寫 --font-size-base）④往生/陽上名單文字＝地址（name-grid input 顯式 --font-size-base）⑤勾指定編號後編號欄移至勾選文字右邊（.keep-number-row flex 同列）⑥地址非必填（前端移 required+星號、後端 Create/Update/InsertShift/BelieverWriteValidator 同步放寬空存空字串，移除 400「請輸入寄件地址」，兩單元測試改斷言接受、integration 改斷言 201）；前後端 build 綠、Application.Tests 201 綠；docs 同步 post-signups/business-rules §12/signup-management/3 份 legacy-coverage；先前 2026-07-18 新增信眾表單客訴改版：地址寄件上/文牒下、名單往生上/陽上下無底色、兩表單 legend 拿掉「最多 6 位」；同日普桌資料卡客訴：template 一樣全印——WorshipCardRenderer.DrawTemplate 畫葫蘆（重用 worship2.png）＋右側標題＋簽名底線，白紙可印，生產字型回掃誤差 ≤0.013cm，回歸鎖 WorshipCard_EmptyContent_StillPrintsTemplate，待實體複驗；同日資料卡客訴改版：template 由程式全印（DrawTemplate 畫標題/簽名底線/窗框/故靈位，白紙可印，372 測試綠、疊圖重合，待實體複驗）；同日文牒三項客訴：往生者沒印（模板選擇 count→slot-based 對齊舊系統，薦牌同步修）＋地址置中預印「臺灣」正下方（Left 25.4/Top 4.9）/加大0.75cm/超長折兩欄＋亡陽姓名0.9cm 比地址大，371 綠、疊圖待實體複驗；同日稍早收據套印座標依客戶樣張 reference/收據.jpg 校正：Name+0.2/Number+0.8右+1.0/Prepay+0.3/年月日+0.5cm 上下聯同步，待實體複驗；同日報表對齊追加：收據 Fee 千分位 N0、Prepay 改「預繳至X年Y」、資料卡/文牒地址改文牒地址（新版誤用郵寄地址，收據封面維持郵寄）＋更正 positions 盲點 10 寫反的 Fee 敘述，365 測試綠；同日收據列印兩項修正：封面頁沒印（客訴）——ReceiptRenderer 補第 2 頁郵寄封面 Zipcode/Address/Name 16pt、空地址也輸出維持頁數，＋年份誤印西元改民國年（-1911 對齊舊 taiwanCalendar.GetYear），360 測試綠＋目視兩頁 OK；批次列印起迄客訴「沒辦法手動輸入」：Playwright 全路徑無法重現→根因鎖定 input[type=number] 丟棄中文輸入法組字且無回饋，新增 appNumericInput directive（CVA、全形轉半形、值維持 number|null）加固全站 12 個數字欄，build+Playwright 驗證過，規則入 coding-style/gotchas；報名維護備註搜尋沒反應：前端 signup.api.ts search() 漏送 scopeRemark 參數補上，後端 LIKE 邏輯本正確；同日右鍵列印普桌/普桌資料卡完全解鎖：前端恆啟用＋後端撤回全部 type-4 限制（422/批次過濾），對齊舊系統選什麼印什麼，357 測試綠＋實測 200；同日稍早普桌六位客訴置中：Base 變體 6 欄 Left +0.1786cm 對齊葫蘆中軸，普桌資料卡同步；先前 2026-07-17 報名編輯 overlay 按確認必 400 修復：編輯模式編號欄恆顯示＋form invalid 不再靜默；同日：載入預繳 500 修復：SignupLogs.Name NOT NULL 被 7/4 稽核的「log 快照留 null」踩中→log Name 改信眾姓名快照，補真實 insert 整合測試，357 測試綠；同日稍早：薦牌列印 3 項客訴修復：5位字太小/陽上超出列印範圍/編號被裁，MatrixLayout 方框內動態排版＋陽上左界 0.5＋編號 Left 0.5，88 測試綠、疊圖 OK 待實體驗收；同日稍早：新增報名表單對齊舊系統四項 v2.2.1：信眾搜尋改常駐 in-form 結果列表（截斷 200 列防卡頓）、地址寄件上/文牒下、名單往生上/陽上下且輸入框無底色、未選信眾送出自動先 POST /believers 建新信眾再報名（修復「沒選信眾無法新增」）；同日追加：結果列表配色/列高對齊報名維護 grid、法會資料提到表單最上方且空表單無捲動，Playwright 實測全過；先前：列印新增「普桌資料卡」worshipcard 全新報表：葫蘆內編號+陽上 6 變體墨跡仿射映射、右側 Phone/Remark 套印、限 type-4、354 測試綠、疊圖 6 變體目視 OK 待實體驗收；新增信眾表單改雙欄密集排版節省空間、四種視窗實測無垂直捲動；普桌列印修正完成：丟字修復+6變體各自座標+每格5字+上下排空格，340 測試綠；先前稽核：實測確認丟字僅 One/Two/Three 變體、Base/Four/Five 有印但座標沿用 Base 矩陣，P1 項精確化；報名編號「插入並順移」新功能：列表右鍵「在此前插入」→ POST /signups/insert-shift，set-based UPDATE +1 順移 + sp_getapplock 與預繳共用，333 測試綠 + Playwright 實機；薦牌實體對位使用者確認 OK，Blocked 項結案；載入預繳對齊稽核修正 4 項：Name/Phone 留 null、配號 nextNo=n+1 對齊舊系統、並行鎖 UPDLOCK+sp_getapplock 真正落地、確認不做預覽；配號抽為 PrepayNumberAllocator 純函式，326 測試綠；新增報名表單改雙欄密集排版節省空間、避免垂直捲動；先前修正全站文字太小/顏色不清楚：WCAG 對比實測 + --c-text-secondary 改深 + 新增 --c-primary-strong 修按鈕文字對比 + 字級再 +1px；2026-07-06 薦牌亡者/陽上矩陣同欄上下排姓名間補全形空白間距；新增 GET /reports/tablet/sample dev-only 端點；2026-07-05 薦牌 OneOne 變體 Y 座標修正))))
