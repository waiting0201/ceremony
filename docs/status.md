...
- 🚧 **薦牌整體左移 2.5cm — 已實作，但實測會讓編號與陽上整組印不出來** — Blocked 2026-08-05
  - Blueprint: [printing-reports-positions.md](blueprints/printing-reports-positions.md) §3「2026-08-05」條
  - **已做**：[TabletRenderer.cs](../backend/src/Ceremony.Infrastructure/Reporting/TabletRenderer.cs) 新增 `GlobalShiftX = -2.5`，套在 `DrawText`，9 變體全欄位共用、欄間相對關係不變、只有一個改點。Infrastructure 135 測試綠。
  - **Blocker**：薦牌頁面 `Margin 0`、內容原自 `Left 0.5` 起排，左移 2.5cm 後編號（−2.0）與全部陽上欄位（−1.69~−0.55）落在 PDF 頁面外 → **整組不印**（200DPI 回掃只剩往者一團 x 2.02~4.42cm）；往者也移出樣板雕花窗框（內緣 4.191~7.163cm）。超出 MediaBox 的內容無法用印表機設定救回。
  - **Waiting on**：使用者確認要 (1) 改小位移量（實際可左移空間 ≈0）、(2) 只左移往者（改 `DeadCenterX`）、還是 (3) 這其實是實印整體偏右的客訴 → 應改查送印端驅動自訂表單尺寸（`PrinterFormMatcher` / Ceremony.PrintForm，115×255mm）。
  - **未加回歸鎖**：位移量定案前不鎖 X。
