...
| 步驟 | 動作 |
|---|---|
| ① 分流 | 用**記事本**開同一台印表機的「內容」。記事本也噴 → 純 Windows／驅動問題（重啟 Print Spooler 服務、重裝驅動），與本程式無關 |
