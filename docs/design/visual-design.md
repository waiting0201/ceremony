...
```css
--font-size-xs: 11px;    /* 小標注 (例：(不須填符號，例：0987654321)) */
--font-size-sm: 12px;    /* 表單欄位 */
--font-size-base: 13px;  /* 主要內容 */
--font-size-md: 14px;    /* 按鈕、強調 */
--font-size-lg: 16px;    /* 區塊標題 */
--font-size-xl: 20px;    /* 頁面標題 */
```

對應舊 WinForms 9pt~12pt（96 DPI 下 9pt ≈ 12px）。
...
| `--c-dead-name-bg` | `#EFDCC4` | 往生名欄位 highlight（飽和度略高的桃色，與 primary-soft 區隔） |
...
- 往生欄背景 `--c-dead-name-bg` (#EFDCC4)（對齊舊 DataGridView DefaultCellStyle）
