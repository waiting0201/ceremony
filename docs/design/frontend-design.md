...
- **批次只等渲染，不取檔**：`BatchPrintService.render()` 回 `{jobId, fileName}`，`/file` 是 one-shot，
  ...
  | 薦牌對位校正版（2026-08-06） | `printSingle('tablet', id, { debugGrid: true })` → `openReportInViewer('/reports/tablet?signupId=…&debugGrid=true')` |

- **對位校正版的 `reportType` 必須維持 `'tablet'`**：紙張 SSoT 與 `X-Report-Page-Size` 都由它決定，
  為了在 UI 上區分而傳一個假型別會讓校正版用錯紙張——**量出來的刻度就沒有意義**。
  入口是報名維護清單右鍵「列印薦牌（對位校正）」（限選 1 筆，不進批次）。用途見
