...
- **儲存格截斷 tooltip**（**2026-08-06 新增**，對齊舊 `DataGridView.ShowCellToolTips`）：欄寬不足導致文字被截斷時，滑鼠停留 500ms（＝舊 `ToolTip.InitialDelay` 預設值）冒出該格完整內容；**沒被截斷就不冒**（舊系統即為此條件，無條件冒會變成每格都跳一模一樣的字）。表頭欄名同規則。實作 [shared/cell-tooltip/](../../frontend/src/app/shared/cell-tooltip/)，掛在 `.vgrid-zone` 上做事件委派而非逐格綁定（虛擬捲動下 row 反覆回收，逐格綁會一直建/銷毀監聽器），報名維護與信眾清單共用。三個實作重點：(1) 儲存格文字**必須包一層 `.vgrid-td-text`**——`.vgrid-td` 是 flex 容器，裸文字節點會變成匿名 flex item，`text-overflow` 掛在容器上不生效（原本只是硬切沒有 `…`），`scrollWidth`/`clientWidth` 也量不到而永遠判成「沒截斷」；(2) 捲動一律關掉 tooltip，否則虛擬捲動把該格拿去畫別列後，tooltip 會指到錯的資料；(3) overlay pane 給 `pointer-events: none`，不然 tooltip 蓋住格線時會吃掉選列與右鍵
