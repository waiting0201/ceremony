...
- **頁面 layout 走垂直堆疊**（不做左右分欄），詳見 [visual-design.md「列印預覽頁面」](visual-design.md#列印預覽頁面reportspreview2026-05-28-重新設計)
  - 最上：**列印排障列**（桌面版限定，與有沒有預覽無關；見下方〈列印排障列〉）
  - 上：緊湊水平表單列（**只有「依編號區間」一種**；mode tabs 與「單筆列印」分頁已於 2026-08-11 移除，見下方〈本頁不收 Signup ID〉）
...
### 本頁不收 Signup ID（**2026-08-11 使用者指定移除「單筆列印」**）

原本 `/reports/preview` 有「單筆列印 / 批次列印」兩個 mode tab，單筆那格要使用者貼上
**Signup ID（GUID）**。但那串值在整套系統的畫面上**任何地方都看不到**——清單顯示的是
年度／法會／編號／姓名，使用者根本無從得知——等於一個只有開發者用得動的入口，
卻佔著這一頁的主要位置。使用者回報「客人根本不知道 Signup ID 是什麼」，指定移除。

- **能力沒有消失，只是入口收斂**：單筆／多筆列印的正式入口是**報名維護清單的右鍵選單**
  （選取列 → 列印 <報表>），六種報表全都有，走 `PrintService.printSingle` / `printBatch`；
  本頁只負責「依編號區間」這一種批次（起迄填同一個編號就是印一筆）
- **`ReportApi.single()` 保留**：它仍是右鍵單筆那條路徑的實作（`PrintService`），
  移除的只是「讓使用者手打 ID」這個 UI
- **副標補一句指路**：原本用那格的人要知道該去哪裡按，不能默默拿掉就算
- **判準：要使用者輸入的識別碼，必須是他在畫面上看得到的那一個。** 內部識別碼（GUID / 流水號）
  可以拿來當程式間的 key，但不該出現在要使用者填的欄位裡

### 送印路徑（**2026-08-02 回歸舊系統形狀：預覽視窗 + Windows 原生列印對話框**）

`core/print/print.service.ts` 是**唯一列印入口**，四個呼叫點（右鍵單筆 / 右鍵多筆 / 編號批次 / 新增後列印）
與報表預覽頁都走它。**形狀對齊舊系統**：程式只把 PDF 開在預覽視窗，送印本身
（印表機／份數／紙張／方向／頁面範圍）全部交給 Windows 原生列印對話框。
自建列印對話框與所有送印參數已於 2026-08-02 整組移除。

| 環境 | 流程 |
|---|---|
| Electron | `ceremony:openReportInViewer(type, apiPath, token)` → 主行程串流取檔 → Chromium PDF 檢視器視窗 → 使用者按工具列 🖨 → 原生 PrintDlgEx |
| 瀏覽器（`ng serve` / 單元測試） | 自己抓 blob → `openPdfInNewTab` |

- **PDF 不進 renderer**：批次合併成一份可達數百 MB，走 IPC 會 renderer + main 各一份。
  例外是報表預覽頁（blob 已在手上）→ `ceremony:openPdfInViewer(type, bytes)`。
- **不要再引入送印參數**：紙張尺寸在後端產 PDF 時就定案（`ReportPageSizes` ＝舊系統的 `DeviceInfo`），
  在送印端二次指定正是 v2.3.7／v2.3.8 兩輪客訴的來源。
- **批次只等渲染，不取檔**：`BatchPrintService.render()` 回 `{jobId, fileName}`，`/file` 是 one-shot，
  留給主行程去串流。`run()`（連 blob 取回）只給瀏覽器與報表預覽頁用。

#### 報表預覽頁的「列印排障列」（桌面版限定）

`/reports/preview` 頁面**上方**（page-header 之下、表單列之上）有一條獨立的 `.trouble-bar`，
三顆按鈕不參與正常流程，是**現場出事時的入口**，非 Electron 環境整條不顯示：

| 按鈕 | 做什麼 |
|---|---|
| 診斷紀錄 | 開 `%APPDATA%/Ceremony/logs` 讓使用者把 `print-*.log` 傳回來 |
| **印表機設定**（2026-08-10） | `rundll32 printui.dll,PrintUIEntry /e /n "<預設印表機>"` ＝ 叫出 Windows 自己的「列印喜好設定」。改一次紙按確定就覆寫掉被改壞的驅動設定 |
| **自動選紙：開／關**（2026-08-10） | 關 ＝ 程式完全不去碰驅動設定（`config.json` 的 `printFormPreselect`）；開 ＝ 恢復，並**同時清掉失敗印表機黑名單** |

> **為什麼這三件事一定要是按鈕**：客戶回報「**找不到那些檔案**」。任何寫成
> 「請到 `%APPDATA%\Ceremony` 刪 xxx.json」的排障步驟，在現場的成功率趨近於零——
> 那種路徑只能寫給遠端協助的人看。凡是要現場執行的復位動作，都必須在 UI 上有入口。

> ⚠️ **不可以掛在預覽工具列裡**（2026-08-11 修正）：2026-08-10 首版把這三顆放在
> `.preview-toolbar`，那條工具列在 `@if (previewUrl())` 之內——**要先產出一份 PDF 才會出現**。
> 但它們要救的故障情境正是「按下列印鈕整個卡死 / 印不出來」，那種時候根本產不出預覽，
> 等於現場永遠按不到。使用者回報「**工具列沒看到**」即此。
> 判準：**復位鍵的可見性不得依賴任何會被故障本身破壞的前提**。

⚠️ 「印表機設定」的印表機名稱**只從 `getPrintersAsync()` 的 `isDefault` 取**，不接受 renderer 傳字串，
而且走參數陣列、**不得用 `shell: true`**——現場名稱常是 `\\PC-王小明\HP LaserJet 1020`，
交給 shell 解析等於把使用者的印表機名餵進命令列語法。純函式 `printerPrefsArgs` 由 spec 鎖住。

「自動選紙」那顆的存在理由：某些驅動（KYOCERA PA2000）被列印前的紙張預選碰過之後，
Windows 原生列印對話框會整個卡死、只能重啟程式。在此之前唯一的關閉手段是
`CEREMONY_PRINTFORM_EXE` 環境變數——**寺方按不到**，每次都要遠端協助。
狀態經 `ceremony:getPrintFormState` / `setPrintFormEnabled` 兩支 IPC，
讀不到（或非桌面版）就整格不顯示，不做假的開關。
決策見 [print-channel-electron.md](../blueprints/print-channel-electron.md) 決策 9d。

  | 情境 | 流程 |
  |---|---|
  | 單筆 | `openReportInViewer('/reports/{type}?signupId=…')` |
  | 批次（不分段，合併成一份） | `POST /reports/batch/jobs` → ProgressOverlay 輪詢 → `openReportInViewer('/reports/batch/jobs/{id}/file')` |
  | 報表預覽頁 | `openPdfInViewer(type, bytes)` |
  | 薦牌對位校正版（**無 UI 入口**，2026-08-08） | `printSingle('tablet', id, { debugGrid: true })` → `openReportInViewer('/reports/tablet?signupId=…&debugGrid=true')` |

- **對位校正版的 `reportType` 必須維持 `'tablet'`**：紙張 SSoT 與 `X-Report-Page-Size` 都由它決定，
  為了在 UI 上區分而傳一個假型別會讓校正版用錯紙張——**量出來的刻度就沒有意義**。用途見
  [printing-reports.md](../blueprints/printing-reports.md)「現場對位校正版」。
- **⚠️ 2026-08-08 起沒有 UI 呼叫端**：報名維護右鍵「列印薦牌（對位校正）」已移除（量測工具不該
  出現在客戶的日常操作面）。`PrintService.printSingle` 的 `debugGrid` 參數與兩支 spec 刻意留著，
  要用就遠端直接打該 URL，或把選單項加回 `signup-list-page.ts`（該處有指回 blueprint 的註解）。

  卡紙續印靠原生對話框的「頁面範圍」，不再有分段面板
  （廢止理由見 [chunked-batch-printing.md](../blueprints/chunked-batch-printing.md)）。
...
- [signup-list-page 右鍵「移動插入至…」](../../frontend/src/app/features/signups/signup-list-page.ts)（**2026-08-21 新增**）：報名維護列表 row context menu 加「移動插入至…」（`actionMoveNumber`，icon `move-vertical`），**恰好選 1 筆且該筆有編號**才啟用。不開報名表單——彈 `ConfirmDialogService.askNumber()`（confirm-dialog 新的 `numberInput` 擴充：訊息下方一格 number input，預填目前編號、值非正整數時確認鈕停用、Enter 等同確認）拿到目標編號後打 `SignupApi.moveNumber()`（`POST /signups/{id}/move-number`），中間區段 ±1 自動遞補。兩個刻意的實作點：(1) **範圍檢查不在前端做**——列表顯示的是搜尋結果、不等於整個群組的編號範圍，前端自行判斷會誤擋，一律由後端在鎖內判定並回帶「目前 N–M」；(2) **成功訊息設在 `search()` 之後**——`search()` 開頭會把 `successMessage` 清成 null，先設會被自己蓋掉（**通則**：報名維護頁任何「成功訊息 + 重查列表」的動作都要先重查、後設訊息。2026-08-21 依此修掉刪除那句「已刪除 N 筆報名資料」——它寫在 `await this.search()` 之前，自寫下來就從未真的顯示過；回歸鎖 1 案）。與「在此前插入」的分工：那支是新增一筆、本支只移位（總筆數不變、不留空號）。見 [signup-management.md](../blueprints/signup-management.md)「移動並重排」、[post-signups-move-number.md](../blueprints/api-endpoints/post-signups-move-number.md)
...
兩個收尾方式（`null` ＝使用者取消；丟 `ApiError` ＝真失敗）：

- `render(req, {title?, detail?}) → Promise<{jobId, fileName} | null>` — **只等渲染完成，不取檔**。
  Electron 走這條，成品由主行程串流下載（`/file` 是 one-shot，renderer 先取走主行程就拿不到）
- `run(req, …) → Promise<ReportPdf | null>` — `render()` + 取檔。瀏覽器與報表預覽頁走這條

內部流程：`createBatchJob` → 開 overlay → 每 250ms 輪詢（`await sleep()` 串接的 `while`，
**不用 `setInterval`** 以免請求疊加）→ `completed` 收尾、`canceled` 回 null、`failed` 丟 `ApiError`。
...
### 窄欄位（`.field` + 固定寬）必須解除 input 的最小寬度（**2026-08-11 客訴**）

客訴「批次列印的編號起訖會重疊、含後續年份的 checkbox 也被壓到」。`.field` 是 `display: grid`，**grid 子項的 `min-width` 預設是 `auto` ＝ 內容的 min-content**；`<input>` 沒設 `size` 時 min-content 就是 UA 的預設 20 字元寬（Chrome 實測含 padding/border 約 **190px**）。因此把欄位寬壓成 `width: 100px` 只縮到 label 的盒子，裡面的 input **完全不縮、直接溢出 90px 蓋到右邊那一欄**——而且與視窗寬無關，一開始就是壞的（實測 1100px 寬照樣重疊，只是使用者在窄視窗才注意到）。

- **凡是欄位寬 < 190px，就要顯式寫 `.field > input, .field > select { min-width: 0; }`**，欄位寬才真的是欄位寬
- 溢出**不會**被裁掉也不會報錯，只是被後面的元素蓋住 ⇒ code review 看不出來，要用實際量測（寬度掃描 + 是否有矩形交疊）才驗得到
- 全域 `.field` **不**加這條：其他頁的欄位是靠 input 的固有寬撐出合理大小，全域解除會讓它們塌掉。這是「窄欄位」的區域規則

### 語意相依的欄位用 `.field-group` 綁成一組（**2026-08-11**）

`flex-wrap: wrap` 的工具列型表單，換行點由剩餘空間決定，會把「編號起」留在上一列、「編號迄」擠到下一列，配對關係在畫面上消失。**成對／主從關係的欄位包一層 `.field-group`（自身也是 flex，`flex: 0 0 auto`）**，整組一起換行。目前用於報表預覽頁批次列印的「民國年 ＋ 含後續年份」與「編號起 ＋ 編號迄」。附帶：inline checkbox 的文字要 `white-space: nowrap`——中文可在任意字間斷行，不鎖住會被壓成直排。
...
- **只做純新增模式**：帶 `signupId` / `fromSignupId` / `insertAt` 的模式各有自己的資料來源，還原草稿會互相打架。⚠ 2026-08-05 起代入新增多一個來源——工具列「新增報名」在**恰好選 1 筆**時也帶 `fromSignupId`（對齊舊 `btnNew_Click`）→ 那一次不還原草稿。草稿本身不會被覆寫（`saveDraft()` 同樣被擋掉），清掉選取再按新增報名即可拿回；取捨與逃生口見 [signup-management.md](../blueprints/signup-management.md)
...
- **儲存格 tooltip**（**2026-08-06 新增，同日改版**）：滑鼠停留 500ms（＝舊 `ToolTip.InitialDelay` 預設值）冒出該格內容，**每一格都冒、不看文字有沒有被欄寬截斷**；唯一例外是空白格（沒有內容可顯示）。表頭欄名同規則。〔沿革：初版對齊舊 `DataGridView.ShowCellToolTips`「只有截斷才冒」，同日**使用者指定改為無條件顯示**——沒截斷的格 tooltip 內容雖與格內文字相同，但使用者要的是「移到哪格都看得到」的一致回饋，不必自己判斷這格會不會冒。〕實作 [shared/cell-tooltip/](../../frontend/src/app/shared/cell-tooltip/)，掛在 `.vgrid-zone` 上做事件委派而非逐格綁定（虛擬捲動下 row 反覆回收，逐格綁會一直建/銷毀監聽器），報名維護與信眾清單共用。三個實作重點：(1) 儲存格文字**必須包一層 `.vgrid-td-text`**——`.vgrid-td` 是 flex 容器，裸文字節點會變成匿名 flex item，`text-overflow` 掛在容器上不生效（原本只是硬切沒有 `…`）；這層 span 同時是 tooltip 的 hover 判定與定位錨點（改成無條件顯示後不再量 `scrollWidth`/`clientWidth`，但 `…` 仍要靠它）；(2) 捲動一律關掉 tooltip，否則虛擬捲動把該格拿去畫別列後，tooltip 會指到錯的資料；(3) overlay pane 給 `pointer-events: none`，不然 tooltip 蓋住格線時會吃掉選列與右鍵
...
> 2026-08-05：工具列「新增報名」鈕採**同一條規則、同一個 handler**（`actionAddFrom`）——恰選 1 筆＝代入該列，沒選或多選＝空白新增（對齊舊 `btnNew_Click`）。代入新增開啟後，`signup-edit-form` 會自動把來源姓名填進搜尋框、跑一次信眾搜尋並選中來源列（對齊舊 `btnNextStep_Click:97-111`）；三層 in-flight guard `prefillToken` / `believerSearchToken` / `pickToken` 確保與使用者手動搜尋、手動改選不打架。代入以 `applyBelieverRow(row, { markDirty: false })` 套用＝與使用者點列共用同一段邏輯，但**維持 pristine**（系統帶的不算使用者輸入）。
