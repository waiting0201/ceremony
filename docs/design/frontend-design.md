...
last_updated: 2026-07-17 (signup-edit-form 對齊舊系統四項：信眾搜尋常駐 in-form 列表、地址寄件上/文牒下、名單往生上/陽上下無底色、未選信眾自動建信眾)
---
...
- [believers-page](../../frontend/src/app/features/believers/believers-page.ts) → `<app-form-overlay>` 包 `<app-believer-edit-form>`（**2026-07-04：改雙欄密集排版節省高度**，比照 signup-edit-form 的 `.form-cols` 手法——左欄「基本資料（類型/姓名/堂號/電話/固定編號）→ 地址（寄件/文牒並排半欄）」／右欄「陽上名單 → 往生名單」；邏輯欄位與驗證不變、僅視覺分欄，DOM 順序＝tab 順序（左欄再右欄）。實測（Playwright + `sa@system.local`）：1280×720／1366×768／1440×900／1920×1080 新增信眾 overlay **完全無垂直/水平捲動**（panel 高固定 ~490px））
