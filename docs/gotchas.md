...
- **判別法**（現場 30 秒，決定往哪修）：用**記事本**開同一台印表機的「內容」。記事本也噴 → 純 Windows／驅動問題（重啟 Print Spooler、重裝驅動）；只有本程式噴 → 是我們寫的 DEVMODE
