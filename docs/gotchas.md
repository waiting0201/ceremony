...
- **打包預設連線改硬編 `DEFAULT_CONFIG`（不是種子檔）**：`192.168.1.151` + sa 密碼直接寫在 [frontend/electron/config.ts](../frontend/electron/config.ts)；首次啟動無 config.json 時 `getStatus` 以 `defaults`（含密碼）回給 `/setup` 預填，使用者按「測試連線」即寫出 config.json。**2026-06-18 決策**：使用者選硬編而非 gitignored 種子檔（曾評估過 `readDefaultConfig` + `build/default-config.json` 路線，已移除）。`jwtKey` 仍由 `writeConfig` 每機自動產生（不在 `DEFAULT_CONFIG` 內）。
- **安裝檔 + repo 都含明文 sa 密碼**：`DEFAULT_CONFIG` 在 `config.ts` → 既進 git 歷史、也被打包進安裝檔（asar 內可解出）→ 安裝檔限內部交付勿外流；偏離 CLAUDE.md 規則 11，為已接受例外（取捨見 [security.md](design/security.md)「打包預設連線」段）。
