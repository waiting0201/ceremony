...
- **預防／做法**：碰到「算起來明明還有餘裕卻仍超出」時，**不要調座標**——先把刻度尺印在同一張紙上反推基準。作法是把既有的 `debugGrid`（1cm 桃紅格線）接成現場入口（`GET /reports/tablet?debugGrid=true`，右鍵「列印薦牌（對位校正）」），現場回報三項：相鄰刻度在實體紙上是否真的 1.00cm（先排除縮放）、預印字上緣落在第幾條線、牌位座視窗下緣落在第幾條線。⚠️ **這類現場量測工具不能 dev-gate**——`debugOverlay` 只給開發機用沒問題，`debugGrid` 擋在 Development 等於工具不存在
