...
last_updated: 2026-07-05 (追加 QuestPDF Margin 裁切依元素類型而定的教訓：Text 不會被裁、Image 會；薦牌 OneOne 變體 Y 座標修正)
---
...
### 薦牌實體對位：座標跟 RDLC 1:1 吻合、PDF 本身無重疊，客戶仍反映列印歪（**部分修正，2026-07-03**）
...
- **⚠️ 仍待辦（需使用者/現場人員操作）**：上面的修正都是「用樣板照片這個新資料源」得到的，仍然是**基於平面照片的估算**，不是實體量測——樣板照片本身是否 100% 對應客戶目前使用的實體牌位座尚未確認。印出修正後版本、按原本方式插入同一個蓮花瓶牌位座，回報「視窗上緣對到第幾條刻度線、下緣對到第幾條」（可搭配 `debugGrid:true`），才能確認這次修正方向正確、或算出更精確的修正量。**在拿到實測數字確認前，不要再進一步憑空微調**——牌位座是實體耗材，猜錯方向等於讓客戶多浪費一次列印
- **要點**：牌位座 / 印章位 / 任何「紙本插入實體外殼」的列印品，座標正確 ≠ 對位正確；PDF 頁面邊界只保證「我們畫的東西彼此不重疊、不超出紙張」，保證不了「紙張放進外殼後，畫的東西落在外殼開窗範圍內」——**有實體樣板照片可量測時，可以先修正「量測後仍明顯超出邊界」這種高把握的落差，但最終仍要有一次實機測試收斂**，不能只憑照片就宣告解決
