...
- **預防**：**溢出只能由「可用高」這個唯一防線管**，字級起點交給業務語意（美觀）決定就好。已撤掉該門檻改為「自動縮到剛好」——長名字反而變大（8 字素 0.7cm vs 舊 0.5cm）且保證不溢出。近親案例：任何「用 A 的代理指標去防 B」的規則，A 與 B 脫鉤的那條路徑就是 bug（同見本檔「用代理指標當條件」條）
