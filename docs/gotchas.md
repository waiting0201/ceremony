...
last_updated: 2026-07-02 (新增薦牌實體對位開放問題 + debugGrid 校正工具)
---
...
### 薦牌實體對位：座標跟 RDLC 1:1 吻合、PDF 本身無重疊，客戶仍反映列印歪（**未解決，待實測**，2026-07-02）
...
- **推論**：問題出在「RDLC 當年校準的牌位座實體尺寸」與「客戶現有牌位座」不一致（供應商換過牌位座樣式、或原始校準本來就有落差），屬於**紙條 vs 牌位座視窗的實體對位問題**，不是排版邏輯錯誤——這種問題**無法只靠看 PDF 或看照片校正**，需要拿實體牌位座量測
...
- **⚠️ 待辦（需使用者/現場人員操作）**：印出 `debugGrid:true` 版本、按原本方式插入同一個蓮花瓶牌位座，回報「視窗上緣對到第幾條刻度線、下緣對到第幾條」，才能算出精確修正量（cm）套回 `TabletRenderer` 座標。**在拿到實測數字前，不要盲猜座標修正**——牌位座是實體耗材，猜錯方向等於讓客戶多浪費一次列印
- **要點**：牌位座 / 印章位 / 任何「紙本插入實體外殼」的列印品，座標正確 ≠ 對位正確；PDF 頁面邊界只保證「我們畫的東西彼此不重疊、不超出紙張」，保證不了「紙張放進外殼後，畫的東西落在外殼開窗範圍內」——這種校正必須有一次實測，程式碼本身推不出來
