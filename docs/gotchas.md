...
- **真因**：`electron/api-stream.ts` 的 `streamApiToFile` 直接 `fs.createWriteStream(destPath)`，而列印通道的 destPath 是 `os.tmpdir()/ceremony-print/<uuid>.pdf` —— **這個目錄是本程式自己造的**，Windows 不會預先存在 → ENOENT。`printPdfBuffer` 那條路徑有 `fs.mkdir(recursive:true)`，單筆／批次那條沒有，兩條路徑的前置條件不一致
- **為什麼長期隱形**：`download.ts` 也用同一個函式，但它的 destPath 來自 SaveDialog（目錄必定存在）；開發機上只要有任何一次走過 `printPdfBuffer`，目錄就被建起來，之後永遠正常
