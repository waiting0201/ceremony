...
last_updated: 2026-08-15 (「資料卡」與「普桌資料卡」兩條補記列表右鍵的互斥停用規則〔全普桌鎖一般資料卡、全非普桌鎖普桌資料卡、混選皆可；預覽頁下拉與後端不受影響〕，見 business-rules-implicit §16.2。先前 2026-08-12 (補上缺漏的「普桌資料卡 / WorshipCard」詞條〔新版 2026-07-04 新增、舊系統無對應 RDLC〕，並在「普桌」與「資料卡」兩條加交叉指引；同時標注「普桌」條目的「menu 僅在搜尋類型=4 才啟用」是**舊系統**行為〔新版 2026-07-18 已解鎖〕。先前 2026-07-21 (堂號/員工類型/固定編號改 per-signup 報名自有覆寫欄（方案 A），報名可編輯只改這筆、view COALESCE 回退信眾值、預繳保號仍讀信眾))
---
...
新版 reportType `datacard`。**普桌報名有另一張**（見下方「普桌資料卡」）；列表右鍵在「選取列全為普桌」時會把本張停用、提示改印普桌資料卡（2026-08-15，§16.2）。
...
- 用途上是「普桌報名的資料卡」：新增報名存檔後按「列印資料卡」，`SignupType = 4` 自動改印這張（2026-08-12，見 [business-rules-implicit §16.1](business-rules-implicit.md)）
- 列表右鍵的兩張資料卡**依報名類型互斥停用**（2026-08-15，見 [§16.2](business-rules-implicit.md)）：全普桌只能選這張、全非普桌只能選一般資料卡、混選兩者皆可；列印預覽頁下拉不套此規則，後端一律不限型別
